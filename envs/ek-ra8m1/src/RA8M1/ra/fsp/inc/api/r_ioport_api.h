...
typedef struct st_ioport_pin_cfg {
  uint32_t pin_cfg;       ///< Pin configuration - Use ioport_cfg_options_t
                          ///< parameters to configure
  bsp_io_port_pin_t pin;  ///< Pin identifier
} ioport_pin_cfg_t;

/** Multiple pin configuration data for loading into registers by
 * R_IOPORT_Open() */
typedef struct st_ioport_cfg {
  uint16_t
      number_of_pins;  ///< Number of pins for which there is configuration data
  ioport_pin_cfg_t const *p_pin_cfg_data;  ///< Pin configuration data
  const void *p_extend;  ///< Pointer to hardware extend configuration
} ioport_cfg_t;

/** IOPORT control block.  Allocate an instance specific control block to pass
 * into the IOPORT API calls.
 */
typedef void ioport_ctrl_t;

/** IOPort driver structure. IOPort functions implemented at the HAL layer will
 * follow this API. */
typedef struct st_ioport_api {
  /** Initialize internal driver data and initial pin configurations.  Called
   * during startup.  Do not call this API during runtime.  Use @ref
   * ioport_api_t::pinsCfg for runtime reconfiguration of multiple pins.
   *
   * @param[in]      p_ctrl     Pointer to control structure. Must be declared
   * by user. Elements set here.
   ...
   */
  fsp_err_t (*pinCfg)(ioport_ctrl_t *const p_ctrl, bsp_io_port_pin_t pin,
                      uint32_t cfg);
   ...
   */
  fsp_err_t (*pinRead)(ioport_ctrl_t *const p_ctrl, bsp_io_port_pin_t pin,
   ...
   */
  fsp_err_t (*pinWrite)(ioport_ctrl_t *const p_ctrl, bsp_io_port_pin_t pin,
   ...
   * @param[in]  direction_values     Value controlling direction of pins on
   * port.
   * @param[in]  mask                 Mask controlling which pins on the port
   * are to be configured.
   */
  fsp_err_t (*portDirectionSet)(ioport_ctrl_t *const p_ctrl, bsp_io_port_t port,
   ...
   * @param[in]  event_data           Data to be written as event data to
   * specified port.
   * @param[in]  mask_value           Each bit set to 1 in the mask corresponds
   * to that bit's value in event data. being written to port.
   */
   ...
   */
  fsp_err_t (*portRead)(ioport_ctrl_t *const p_ctrl, bsp_io_port_t port,
   ...
   * are written to.
   */
  fsp_err_t (*portWrite)(ioport_ctrl_t *const p_ctrl, bsp_io_port_t port,
...
typedef struct st_ioport_instance {
  ioport_ctrl_t
      *p_ctrl;  ///< Pointer to the control structure for this instance
  ioport_cfg_t const
