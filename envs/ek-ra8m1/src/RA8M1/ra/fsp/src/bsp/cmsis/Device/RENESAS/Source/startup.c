...
BSP_SECTION_FLASH_GAP void Default_Handler(void) {
  /** A error has occurred. The user will need to investigate the cause. Common
   * problems are stack corruption or use of an invalid pointer. Use the Fault
   * Status window in e2 studio or manually check the fault status registers for
   * more information.
   */
...
/* Main stack */
static uint8_t
    g_main_stack[BSP_CFG_STACK_MAIN_BYTES +
                 BSP_TZ_STACK_SEAL_SIZE] BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT)
        BSP_PLACE_IN_SECTION(BSP_SECTION_STACK);

/* Heap */
#if (BSP_CFG_HEAP_BYTES > 0)

BSP_DONT_REMOVE static uint8_t g_heap[BSP_CFG_HEAP_BYTES] BSP_ALIGN_VARIABLE(
    BSP_STACK_ALIGNMENT) BSP_PLACE_IN_SECTION(BSP_SECTION_HEAP);
#endif

/* All system exceptions in the vector table are weak references to
 * Default_Handler. If the user wishes to handle these exceptions in their code
 * they should define their own function with the same name.
 */
...
/* Vector table. */
BSP_DONT_REMOVE const exc_ptr_t
    __Vectors[BSP_CORTEX_VECTOR_TABLE_ENTRIES] BSP_PLACE_IN_SECTION(
        BSP_SECTION_FIXED_VECTORS) = {
        (exc_ptr_t)(&g_main_stack[0] +
                    BSP_CFG_STACK_MAIN_BYTES), /*      Initial Stack Pointer */
