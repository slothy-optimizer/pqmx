 ...
 **********************************************************************************************************************/
#include <string.h>
#if defined(__GNUC__) && defined(__llvm__) && !defined(__ARMCC_VERSION) && \
    !defined(__CLANG_TIDY__)
#include <picotls.h>
#endif
#include "bsp_api.h"
...
/* Startup value for CCR to enable instruction cache, branch prediction and LOB
 * extension */
#define CCR_CACHE_ENABLE (0x000E0201)

/* Value to write to OAD register of MPU stack monitor to enable NMI when a
...
static void memcpy_64(uint64_t *destination, const uint64_t *source,
                      size_t count);

#endif
#if BSP_FEATURE_BSP_HAS_DTCM
static void memset_64(uint64_t *destination, const uint64_t value,
                      size_t count);
...
BSP_SECTION_FLASH_GAP void SystemInit(void) {
   ...
   * See erratum 3175626 and 3190818 in the Cortex-M85 AT640 and Cortex-M85 with
   * FPU AT641 Software Developer Errata Notice (Date of issue: March 07, 2024,
   * Document version: 13.0, Document ID: SDEN-2236668). */
  ...
  *p_main_stack_top = BSP_TZ_STACK_SEAL_VALUE;
   ...
   * Set the Secure/Non-Secure VTOR to the vector table address based on the
   * build. This is skipped for non-secure projects because SCB_NS->VTOR is set
   * by the secure project before the non-secure project runs. */
   ...
   * VBTCR1.BPWSWSTP. Reference section 11.2.1 "VBATT Control Register 1
   * (VBTCR1)" and Figure 11.2 "Setting flow of the VBTCR1.BPWSWSTP bit" in the
   * RA4M1 manual R01UM0007EU0110. This must be done before bsp_clock_init
   * because LOCOCR, LOCOUTCR, SOSCCR, and SOMCR cannot be accessed until
   * VBTSR.VBTRVLD is set. */
  R_SYSTEM->VBTCR1 = 1U;
  ...
  /* Set SPEEN bit to enable NMI on stack monitor exception. NMIER bits cannot
   * be cleared after reset, so no need to read-modify-write. */
  ...
  /* Zero out BSS */
#if defined(__ARMCC_VERSION)
  memset((uint8_t *)&Image$$BSS$$ZI$$Base, 0U,
         (uint32_t)&Image$$BSS$$ZI$$Length);
#elif defined(__GNUC__)
  memset(&__bss_start__, 0U,
         ((uint32_t)&__bss_end__ - (uint32_t)&__bss_start__));
#elif defined(__ICCARM__)
  memset((uint32_t *)__section_begin(".bss"), 0U,
         (uint32_t)__section_size(".bss"));
#endif

  /* Copy initialized RAM data from ROM to RAM. */
#if defined(__ARMCC_VERSION)
  memcpy((uint8_t *)&Image$$DATA$$Base, (uint8_t *)&Load$$DATA$$Base,
         (uint32_t)&Image$$DATA$$Length);
#elif defined(__GNUC__)
...
#pragma section = "__DLIB_PERTHREAD"
...
#if defined(__GNUC__) && defined(__llvm__) && !defined(__CLANG_TIDY__) && \
  ...
  for (int32_t i = 0; i < count; i++) {
    void (*p_init_func)(void) =
        (void (*)(void))((uint32_t)&Image$$INIT_ARRAY$$Base +
                         (uint32_t)Image$$INIT_ARRAY$$Base[i]);
    p_init_func();
  }
  ...
  /* For TZ project, it should be called by the secure application, whether RTC
   * module is to be configured as secure or not. */
  ...
  R_PMISC->PWPRS = 1U << BSP_IO_PWPR_PFSWE_OFFSET;  ///< Set PFSWE bit - writing
                                                    ///< to PFS register enabled
#else
  R_PMISC->PWPR = 0;  ///< Clear BOWI bit - writing to PFSWE bit enabled
  R_PMISC->PWPR = 1U << BSP_IO_PWPR_PFSWE_OFFSET;  ///< Set PFSWE bit - writing
                                                   ///< to PFS register enabled
#endif
#endif
  ...
  if ((((0 == R_SYSTEM->PGCSAR) && FSP_PRIV_TZ_USE_SECURE_REGS) ||
       ((1 == R_SYSTEM->PGCSAR) && BSP_TZ_NONSECURE_BUILD)) &&
      (0 != R_SYSTEM->PDCTRGD)) {
    ...
    FSP_HARDWARE_REGISTER_WAIT(
        (R_SYSTEM->PDCTRGD &
         (R_SYSTEM_PDCTRGD_PDCSF_Msk | R_SYSTEM_PDCTRGD_PDPGSF_Msk)),
...
static void bsp_reset_trng_circuit(void) {
  ...
  /* Release register protection for low power modes (per RA2A1 User's Manual
   * (R01UH0888EJ0100) Figure 11.13 "Example of initial setting flow for an
   * unused circuit") */
  R_BSP_RegisterProtectDisable(BSP_REG_PROTECT_OM_LPC_BATT);
  ...
  R_BSP_MODULE_START(
      FSP_IP_TRNG,
      0);  ///< TRNG Module Stop needs to be started/stopped for RA2 series.
#elif BSP_FEATURE_BSP_HAS_SCE5
  R_BSP_MODULE_START(
      FSP_IP_SCE,
      0);  ///< TRNG Module Stop needs to be started/stopped for RA4 series.
#else
...
#error "BSP_FEATURE_BSP_RESET_TRNG is defined but not handled."
  ...
  /* Reapply register protection for low power modes (per RA2A1 User's Manual
   * (R01UH0888EJ0100) Figure 11.13 "Example of initial setting flow for an
   * unused circuit") */
  R_BSP_RegisterProtectEnable(BSP_REG_PROTECT_OM_LPC_BATT);
}
...
static void memcpy_64(uint64_t *destination, const uint64_t *source,
  ...
  /* Suppress IAR warning: "Error[Pe550]: variable "temp" was set but never
   * used" */
  /* "temp" triggers this warning when it lacks an early-clobber modifier, which
   * was removed to allow register reuse with "count". */
  (void)temp;
}
...
static void bsp_init_itcm(void) {
  ...
  count =
      ((uint32_t)&Load$$ITCM_PAD$$Limit - (uint32_t)&Load$$ITCM_DATA$$Base) /
      sizeof(uint64_t);
#elif defined(__GNUC__)
  itcm_destination = (uint64_t *)&__itcm_data_start;
  itcm_source = (uint64_t *)&__itcm_data_init_start;
  count =
      ((uint32_t)&__itcm_data_init_end - (uint32_t)&__itcm_data_init_start) /
      sizeof(uint64_t);
#elif defined(__ICCARM__)
  itcm_destination = (uint64_t *)&ITCM_DATA$$Base;
  itcm_source = (uint64_t *)&ITCM_DATA_INIT$$Base;
  count = ((uint32_t)&ITCM_DATA_INIT$$Limit - (uint32_t)&ITCM_DATA_INIT$$Base) /
          sizeof(uint64_t);
#endif
...
static void bsp_init_dtcm(void) {
  ...
  count =
      ((uint32_t)&Load$$DTCM_PAD$$Limit - (uint32_t)&Load$$DTCM_DATA$$Base) /
      sizeof(uint64_t);
  dtcm_zero_destination = (uint64_t *)&Image$$DTCM_BSS$$Base;
  count_zero = ((uint32_t)&Image$$DTCM_BSS_PAD$$ZI$$Limit -
                (uint32_t)&Image$$DTCM_BSS$$Base) /
               sizeof(uint64_t);
#elif defined(__GNUC__)
  dtcm_destination = (uint64_t *)&__dtcm_data_start;
  dtcm_source = (uint64_t *)&__dtcm_data_init_start;
  count =
      ((uint32_t)&__dtcm_data_init_end - (uint32_t)&__dtcm_data_init_start) /
      sizeof(uint64_t);
  dtcm_zero_destination = (uint64_t *)&__dtcm_bss_start;
  count_zero = ((uint32_t)&__dtcm_bss_end - (uint32_t)&__dtcm_bss_start) /
               sizeof(uint64_t);
#elif defined(__ICCARM__)
  dtcm_destination = (uint64_t *)&DTCM_DATA$$Base;
  dtcm_source = (uint64_t *)&DTCM_DATA_INIT$$Base;
  count = ((uint32_t)&DTCM_DATA_INIT$$Limit - (uint32_t)&DTCM_DATA_INIT$$Base) /
          sizeof(uint64_t);
  dtcm_zero_destination = (uint64_t *)&DTCM_BSS$$Base;
  count_zero = ((uint32_t)&DTCM_BSS$$Limit - (uint32_t)&DTCM_BSS$$Base) /
               sizeof(uint64_t);
#endif
...
static void bsp_init_mpu(void) {
  ...
  /* A region start address and end address must each be aligned to 32 bytes. A
   * region must be a minimum of 32 bytes to be valid. */
  ...
  const ARM_MPU_Region_t bsp_mpu_regions[] = {
      ...
      {.RBAR = ARM_MPU_RBAR(nocache_sdram_start, ARM_MPU_SH_NON, 0U, 0U, 1U),
       .RLAR =
           ARM_MPU_RLAR((nocache_sdram_end - ARMV8_MPU_REGION_MIN_SIZE), 0U)}};

  /* Initialize MPU_MAIR0 and MPU_MAIR1 from attributes table. */
  uint8_t num_attr =
      (sizeof(bsp_mpu_mair_attributes) / sizeof(bsp_mpu_mair_attributes[0]));
  ...
  for (uint8_t i = 0; i < num_regions; i++) {
    ...
    if ((((rlar & MPU_RLAR_LIMIT_Msk) >> MPU_RLAR_LIMIT_Pos) +
         ARMV8_MPU_REGION_MIN_SIZE) >
   ...
   * Provide MPU_CTRL_PRIVDEFENA_Msk to ARM_MPU_Enable() to set
   * MPU_CTRL.PRIVDEFENA.
   */
