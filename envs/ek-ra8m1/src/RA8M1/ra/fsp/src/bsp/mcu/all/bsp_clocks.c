...
#define BSP_PRV_SRAM_UNLOCK                  \
  (((BSP_FEATURE_CGC_SRAMPRCR_KW_VALUE)      \
    << BSP_FEATURE_CGC_SRAMPRCR_KW_OFFSET) | \
   0x1U)
#define BSP_PRV_SRAM_LOCK                    \
  (((BSP_FEATURE_CGC_SRAMPRCR_KW_VALUE)      \
    << BSP_FEATURE_CGC_SRAMPRCR_KW_OFFSET) | \
   0x0U)

/* Calculate value to write to MOMCR/CMC (MODRV controls main clock drive
 * strength and MOSEL determines the source of the main oscillator). */
#if BSP_FEATURE_CGC_MODRV_MASK
#define BSP_PRV_MODRV                                     \
  ((CGC_MAINCLOCK_DRIVE << BSP_FEATURE_CGC_MODRV_SHIFT) & \
...
#define BSP_PRV_MOSEL \
  (BSP_CLOCK_CFG_MAIN_OSC_CLOCK_SOURCE << R_SYSTEM_MOMCR_MOSEL_Pos)
...
#define BSP_PRV_MOSEL \
  (3U << R_SYSTEM_CMC_MOSEL_Pos)  // External clock input mode
#else
...
/* Calculate value to write to CMC (SODRV controls sub-clock oscillator drive
 * capability and SOSEL determines the source of the sub-clock oscillator). */
#if (0 == BSP_CLOCK_CFG_SUBCLOCK_DRIVE)
#define BSP_PRV_SODRV \
  (1U << R_SYSTEM_CMC_SODRV_Pos)  // Sub-Clock Oscillator Drive Capability
                                  // Normal mode
#elif (1 == BSP_CLOCK_CFG_SUBCLOCK_DRIVE)
#define BSP_PRV_SODRV \
  (0U << R_SYSTEM_CMC_SODRV_Pos)  // Sub-Clock Oscillator Drive Capability Low
                                  // Power Mode 1
#else
#define BSP_PRV_SODRV (BSP_CLOCK_CFG_SUBCLOCK_DRIVE << R_SYSTEM_CMC_SODRV_Pos)
#endif
#define BSP_PRV_CMC_SOSC                                          \
  (BSP_PRV_SODRV |                                                \
...
#define BSP_PRV_OSMC \
  (0U << R_SYSTEM_OSMC_WUTMMCK0_Pos)  // Use Sub-clock oscillator (SOSC) as
                                      // Subsystem Clock (FSXP) source.
#elif (BSP_CLOCKS_SOURCE_CLOCK_LOCO == BSP_CFG_FSXP_SOURCE)
#define BSP_PRV_OSMC \
  (1U                \
   << R_SYSTEM_OSMC_WUTMMCK0_Pos)  // Use Low-speed on-chip oscillator clock
                                   // (LOCO) as Subsystem Clock (FSXP) source.
#endif
#endif
...
/* Stop interval of at least 5 SOSC clock cycles between stop and restart of
 * SOSC. Calculated based on 8Mhz of MOCO clock. */
...
#define BSP_PRV_PERIPHERAL_CLK_REQ_BIT_MASK \
  (1U << BSP_PRV_PERIPHERAL_CLK_REQ_BIT_POS)
#define BSP_PRV_PERIPHERAL_CLK_RDY_BIT_POS (7U)
#define BSP_PRV_PERIPHERAL_CLK_RDY_BIT_MASK \
  (1U << BSP_PRV_PERIPHERAL_CLK_RDY_BIT_POS)
...
/* Some MCUs have a requirement that bits 18-16 be set to the same value as the
 * bits for configuring the PCLKB divisor. */
...
#define BSP_PRV_STARTUP_SCKDIVCR                                               \
  (BSP_PRV_STARTUP_SCKDIVCR_ICLK_BITS | BSP_PRV_STARTUP_SCKDIVCR_PCLKE_BITS |  \
   BSP_PRV_STARTUP_SCKDIVCR_PCLKD_BITS | BSP_PRV_STARTUP_SCKDIVCR_PCLKC_BITS | \
   BSP_PRV_STARTUP_SCKDIVCR_PCLKB_BITS | BSP_PRV_STARTUP_SCKDIVCR_PCLKA_BITS | \
...
#define BSP_PRV_PLLCCR                                   \
  ((((BSP_CFG_PLL_MUL & BSP_PRV_PLLCCR_PLLMUL_MASK)      \
...
#define BSP_PRV_PLLCCR                                      \
  ((BSP_PRV_PLLCCR2_PLLMUL & BSP_PRV_PLLCCR2_PLLMUL_MASK) | \
...
#define BSP_PRV_PLLCCR_PLSRCSEL_BIT (4)  // PLSRCSEL in PLLCCR starts at bit 4
#define BSP_PRV_PLLCCR                                         \
  ((((BSP_CFG_PLL_MUL & BSP_PRV_PLL_MUL_CFG_MACRO_PLLMUL_MASK) \
     << BSP_PRV_PLLCCR_PLLMULNF_BIT) |                         \
    (BSP_PRV_PLSRCSEL << BSP_PRV_PLLCCR_PLSRCSEL_BIT)) |       \
   BSP_CFG_PLL_DIV)
#define BSP_PRV_PLLCCR2_PLL_DIV_MASK \
  (0x0F)  // PLL DIV in PLLCCR2/PLL2CCR2 is 4 bits wide
#define BSP_PRV_PLLCCR2_PLL_DIV_Q_BIT \
  (4)  // PLL DIV Q in PLLCCR2/PLL2CCR2 starts at bit 4
#define BSP_PRV_PLLCCR2_PLL_DIV_R_BIT \
  (8)  // PLL DIV R in PLLCCR2/PLL2CCR2 starts at bit 8
#define BSP_PRV_PLLCCR2                              \
  (((BSP_CFG_PLODIVR & BSP_PRV_PLLCCR2_PLL_DIV_MASK) \
...
#define BSP_PRV_PLL2CCR                                          \
  ((((BSP_CFG_PLL2_MUL & BSP_PRV_PLL2_MUL_CFG_MACRO_PLLMUL_MASK) \
     << BSP_PRV_PLL2CCR_PLLMULNF_BIT) |                          \
    (BSP_PRV_PL2SRCSEL << BSP_PRV_PLL2CCR_PLSRCSEL_BIT)) |       \
   BSP_CFG_PLL2_DIV)
#define BSP_PRV_PLL2CCR2_PLL_DIV_MASK \
  (0x0F)  // PLL DIV in PLL2CCR2 is 4 bits wide
#define BSP_PRV_PLL2CCR2_PLL_DIV_Q_BIT \
  (4)  // PLL DIV Q in PLL2CCR2 starts at bit 4
#define BSP_PRV_PLL2CCR2_PLL_DIV_R_BIT \
  (8)  // PLL DIV R in PLL2CCR2 starts at bit 8
#define BSP_PRV_PLL2CCR2                               \
  (((BSP_CFG_PL2ODIVR & BSP_PRV_PLL2CCR2_PLL_DIV_MASK) \
...
#else
#define BSP_PRV_PLL2CCR                                 \
  ((BSP_CFG_PLL2_MUL << R_SYSTEM_PLL2CCR_PLL2MUL_Pos) | \
...
#define BSP_PRV_MAIN_OSC_USED (1)
#elif defined(BSP_CFG_CANFDCLK_SOURCE) && \
    (BSP_CFG_CANFDCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)
#define BSP_PRV_MAIN_OSC_USED (1)
#elif defined(BSP_CFG_PLL_SOURCE) &&                            \
    (BSP_CFG_PLL_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC) && \
    BSP_PRV_PLL_USED
#define BSP_PRV_MAIN_OSC_USED (1)
#define BSP_PRV_STABILIZE_MAIN_OSC (1)
#elif defined(BSP_CFG_PLL2_SOURCE) &&                            \
    (BSP_CFG_PLL2_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC) && \
    BSP_PRV_PLL2_USED
#define BSP_PRV_MAIN_OSC_USED (1)
#define BSP_PRV_STABILIZE_MAIN_OSC (1)
#elif defined(BSP_CFG_CLKOUT_SOURCE) && \
    (BSP_CFG_CLKOUT_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)
#define BSP_PRV_MAIN_OSC_USED (1)
#elif defined(BSP_CFG_SCISPICLK_SOURCE) && \
    (BSP_CFG_SCISPICLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)
#define BSP_PRV_MAIN_OSC_USED (1)
#elif defined(BSP_CFG_SPICLK_SOURCE) && \
    (BSP_CFG_SPICLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)
#define BSP_PRV_MAIN_OSC_USED (1)
#elif defined(BSP_CFG_SCICLK_SOURCE) && \
    (BSP_CFG_SCICLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)
#define BSP_PRV_MAIN_OSC_USED (1)
#elif defined(BSP_CFG_CANFDCLK_SOURCE) && \
    (BSP_CFG_CANFDCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)
#define BSP_PRV_MAIN_OSC_USED (1)
#elif defined(BSP_CFG_GPTCLK_SOURCE) && \
    (BSP_CFG_GPTCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)
#define BSP_PRV_MAIN_OSC_USED (1)
#elif defined(BSP_CFG_IICCLK_SOURCE) && \
    (BSP_CFG_IICCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)
#define BSP_PRV_MAIN_OSC_USED (1)
#elif defined(BSP_CFG_CECCLK_SOURCE) && \
    (BSP_CFG_CECCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)
#define BSP_PRV_MAIN_OSC_USED (1)
#elif defined(BSP_CFG_I3CCLK_SOURCE) && \
    (BSP_CFG_I3CCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)
#define BSP_PRV_MAIN_OSC_USED (1)
#elif defined(BSP_CFG_LCDCLK_SOURCE) && \
    (BSP_CFG_LCDCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)
#define BSP_PRV_MAIN_OSC_USED (1)
#elif defined(BSP_CFG_U60CLK_SOURCE) && \
    (BSP_CFG_U60CLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)
#define BSP_PRV_MAIN_OSC_USED (1)
#elif defined(BSP_CFG_OCTA_SOURCE) && \
    (BSP_CFG_OCTA_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)
#define BSP_PRV_MAIN_OSC_USED (1)
#elif defined(BSP_CFG_SDADC_CLOCK_SOURCE) && \
    (BSP_CFG_SDADC_CLOCK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)
#define BSP_PRV_MAIN_OSC_USED (1)
#elif defined(BSP_CFG_UARTA_CLOCK_SOURCE) && \
    (BSP_CFG_UARTA_CLOCK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)
#define BSP_PRV_MAIN_OSC_USED (1)
#elif defined(BSP_CFG_TML_FITL0_SOURCE) && \
    (BSP_CFG_TML_FITL0_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)
#define BSP_PRV_MAIN_OSC_USED (1)
#elif defined(BSP_CFG_TML_FITL1_SOURCE) && \
    (BSP_CFG_TML_FITL1_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)
#define BSP_PRV_MAIN_OSC_USED (1)
#else
#define BSP_PRV_MAIN_OSC_USED (0)
#endif

/* All clocks with configurable source can use HOCO except the CECCLK and
 * I3CCLK. */
#if (BSP_CFG_CLOCK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_HOCO)
#define BSP_PRV_HOCO_USED (1)
#define BSP_PRV_STABILIZE_HOCO (1)
#elif defined(BSP_CFG_PLL_SOURCE) && \
    (BSP_CFG_PLL_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_HOCO) && BSP_PRV_PLL_USED
#define BSP_PRV_HOCO_USED (1)
#define BSP_PRV_STABILIZE_HOCO (1)
#elif defined(BSP_CFG_PLL2_SOURCE) && \
    (BSP_CFG_PLL2_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_HOCO) && BSP_PRV_PLL2_USED
#define BSP_PRV_HOCO_USED (1)
#define BSP_PRV_STABILIZE_HOCO (1)
#elif defined(BSP_CFG_UCK_SOURCE) && BSP_FEATURE_BSP_HAS_USB_CLOCK_REQ && \
...
#define BSP_PRV_HOCO_USED (1)
#elif defined(BSP_CFG_CLKOUT_SOURCE) && \
    (BSP_CFG_CLKOUT_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_HOCO)
#define BSP_PRV_HOCO_USED (1)
#elif defined(BSP_CFG_SCISPICLK_SOURCE) && \
    (BSP_CFG_SCISPICLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_HOCO)
#define BSP_PRV_HOCO_USED (1)
#elif defined(BSP_CFG_SPICLK_SOURCE) && \
    (BSP_CFG_SPICLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_HOCO)
#define BSP_PRV_HOCO_USED (1)
#elif defined(BSP_CFG_SCICLK_SOURCE) && \
    (BSP_CFG_SCICLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_HOCO)
#define BSP_PRV_HOCO_USED (1)
#elif defined(BSP_CFG_CANFDCLK_SOURCE) && \
    (BSP_CFG_CANFDCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_HOCO)
#define BSP_PRV_HOCO_USED (1)
#elif defined(BSP_CFG_GPTCLK_SOURCE) && \
    (BSP_CFG_GPTCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_HOCO)
#define BSP_PRV_HOCO_USED (1)
#elif defined(BSP_CFG_IICCLK_SOURCE) && \
    (BSP_CFG_IICCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_HOCO)
#define BSP_PRV_HOCO_USED (1)
#elif defined(BSP_CFG_LCDCLK_SOURCE) && \
    (BSP_CFG_LCDCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_HOCO)
#define BSP_PRV_HOCO_USED (1)
#elif defined(BSP_CFG_U60CLK_SOURCE) && \
    (BSP_CFG_U60CLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_HOCO)
#define BSP_PRV_HOCO_USED (1)
#elif defined(BSP_CFG_OCTA_SOURCE) && \
    (BSP_CFG_OCTA_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_HOCO)
#define BSP_PRV_HOCO_USED (1)
#elif defined(BSP_CFG_SDADC_CLOCK_SOURCE) && \
    (BSP_CFG_SDADC_CLOCK_SOURCE == BSP_CLOCKS_SOURCE_HOCO)
#define BSP_PRV_HOCO_USED (1)
#elif defined(BSP_CFG_UARTA_CLOCK_SOURCE) && \
    (BSP_CFG_UARTA_CLOCK_SOURCE == BSP_CLOCKS_SOURCE_HOCO)
#define BSP_PRV_HOCO_USED (1)
#elif defined(BSP_CFG_TML_FITL0_SOURCE) && \
    (BSP_CFG_TML_FITL0_SOURCE == BSP_CLOCKS_SOURCE_HOCO)
#define BSP_PRV_HOCO_USED (1)
#elif defined(BSP_CFG_TML_FITL1_SOURCE) && \
    (BSP_CFG_TML_FITL1_SOURCE == BSP_CLOCKS_SOURCE_HOCO)
#define BSP_PRV_HOCO_USED (1)
#else
...
#define BSP_PRV_MOCO_USED (1)
#elif defined(BSP_CFG_SCISPICLK_SOURCE) && \
    (BSP_CFG_SCISPICLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MOCO)
#define BSP_PRV_MOCO_USED (1)
#elif defined(BSP_CFG_SPICLK_SOURCE) && \
    (BSP_CFG_SPICLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MOCO)
#define BSP_PRV_MOCO_USED (1)
#elif defined(BSP_CFG_SCICLK_SOURCE) && \
    (BSP_CFG_SCICLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MOCO)
#define BSP_PRV_MOCO_USED (1)
#elif defined(BSP_CFG_CANFDCLK_SOURCE) && \
    (BSP_CFG_CANFDCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MOCO)
#define BSP_PRV_MOCO_USED (1)
#elif defined(BSP_CFG_GPTCLK_SOURCE) && \
    (BSP_CFG_GPTCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MOCO)
#define BSP_PRV_MOCO_USED (1)
#elif defined(BSP_CFG_IICCLK_SOURCE) && \
    (BSP_CFG_IICCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MOCO)
#define BSP_PRV_MOCO_USED (1)
#elif defined(BSP_CFG_I3CCLK_SOURCE) && \
    (BSP_CFG_I3CCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MOCO)
#define BSP_PRV_MOCO_USED (1)
#elif defined(BSP_CFG_LCDCLK_SOURCE) && \
    (BSP_CFG_LCDCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MOCO)
#define BSP_PRV_MOCO_USED (1)
#elif defined(BSP_CFG_U60CLK_SOURCE) && \
    (BSP_CFG_U60CLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MOCO)
#define BSP_PRV_MOCO_USED (1)
#elif defined(BSP_CFG_OCTA_SOURCE) && \
    (BSP_CFG_OCTA_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MOCO)
#define BSP_PRV_MOCO_USED (1)
#elif defined(BSP_CFG_UARTA_CLOCK_SOURCE) && \
    (BSP_CFG_UARTA_CLOCK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MOCO)
#define BSP_PRV_MOCO_USED (1)
#elif defined(BSP_CFG_TML_FITL0_SOURCE) && \
    (BSP_CFG_TML_FITL0_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MOCO)
#define BSP_PRV_MOCO_USED (1)
#elif defined(BSP_CFG_TML_FITL1_SOURCE) && \
    (BSP_CFG_TML_FITL1_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_MOCO)
#define BSP_PRV_MOCO_USED (1)
#else
#define BSP_PRV_MOCO_USED (0)
#endif

/* All clocks with configurable source except UCK, CANFD, LCDCLK, USBHSCLK,
 * I3CCLK and PLL can use LOCO. */
#if (BSP_CFG_CLOCK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_LOCO)
#define BSP_PRV_LOCO_USED (1)
#define BSP_PRV_STABILIZE_LOCO (1)
#elif defined(BSP_CFG_CLKOUT_SOURCE) && \
    (BSP_CFG_CLKOUT_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_LOCO)
#define BSP_PRV_LOCO_USED (1)
#elif defined(BSP_CFG_SCISPICLK_SOURCE) && \
    (BSP_CFG_SCISPICLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_LOCO)
#define BSP_PRV_LOCO_USED (1)
#elif defined(BSP_CFG_SPICLK_SOURCE) && \
    (BSP_CFG_SPICLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_LOCO)
#define BSP_PRV_LOCO_USED (1)
#elif defined(BSP_CFG_SCICLK_SOURCE) && \
    (BSP_CFG_SCICLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_LOCO)
#define BSP_PRV_LOCO_USED (1)
#elif defined(BSP_CFG_CANFDCLK_SOURCE) && \
    (BSP_CFG_CANFDCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_LOCO)
#define BSP_PRV_LOCO_USED (1)
#elif defined(BSP_CFG_GPTCLK_SOURCE) && \
    (BSP_CFG_GPTCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_LOCO)
#define BSP_PRV_LOCO_USED (1)
#elif defined(BSP_CFG_IICCLK_SOURCE) && \
    (BSP_CFG_IICCLK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_LOCO)
#define BSP_PRV_LOCO_USED (1)
#elif defined(BSP_CFG_OCTA_SOURCE) && \
    (BSP_CFG_OCTA_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_LOCO)
#define BSP_PRV_LOCO_USED (1)
#elif (defined(BSP_CFG_UARTA_CLOCK_SOURCE) &&                           \
       (BSP_CFG_UARTA_CLOCK_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_FSXP)) && \
    (defined(BSP_CFG_FSXP_SOURCE) &&                                    \
     (BSP_CFG_FSXP_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_LOCO))
#define BSP_PRV_LOCO_USED (1)
#elif (defined(BSP_CFG_FSXP_SOURCE) && \
       (BSP_CFG_FSXP_SOURCE == BSP_CLOCKS_SOURCE_CLOCK_LOCO))
#define BSP_PRV_LOCO_USED (1)
#else
...
/* Determine the optimal operating speed mode to apply after clock configuration
 * based on the startup clock frequency. */
...
#if (BSP_FEATURE_BSP_HAS_CANFD_CLOCK &&                                \
     (BSP_CFG_CANFDCLK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED) &&         \
     (BSP_CFG_CANFDCLK_SOURCE != BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC)) || \
    (BSP_FEATURE_BSP_HAS_SCISPI_CLOCK &&                               \
     (BSP_CFG_SCISPICLK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)) ||       \
    (BSP_FEATURE_BSP_HAS_SCI_CLOCK &&                                  \
     (BSP_CFG_SCICLK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)) ||          \
    (BSP_FEATURE_BSP_HAS_SPI_CLOCK &&                                  \
     (BSP_CFG_SPICLK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)) ||          \
    (BSP_FEATURE_BSP_HAS_GPT_CLOCK &&                                  \
     (BSP_CFG_GPTCLK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)) ||          \
    (BSP_FEATURE_BSP_HAS_IIC_CLOCK &&                                  \
     (BSP_CFG_IICCLK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)) ||          \
    (BSP_FEATURE_BSP_HAS_CEC_CLOCK &&                                  \
     (BSP_CFG_CECCLK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)) ||          \
    (BSP_FEATURE_BSP_HAS_I3C_CLOCK &&                                  \
     (BSP_CFG_I3CCLK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)) ||          \
    (BSP_FEATURE_BSP_HAS_USB60_CLOCK_REQ &&                            \
     (BSP_CFG_U60CK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)) ||           \
    (BSP_FEATURE_BSP_HAS_LCD_CLOCK &&                                  \
     (BSP_CFG_LCDCLK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED))
...
static void bsp_clock_set_postchange(uint32_t updated_freq_hz,
                                     uint8_t new_rom_wait_state);
...
static void bsp_prv_operating_mode_flmode_set(uint8_t operating_mode);
    ...
    (BSP_CFG_CLKOUT_SOURCE != BSP_CFG_CLOCK_SOURCE)
...
/* This array stores the clock frequency of each system clock. This section of
 * RAM should not be initialized by the C runtime environment. This is
 * initialized and used in bsp_clock_init, which is called before the C runtime
 * environment is initialized. */
static uint32_t g_clock_freq[BSP_PRV_NUM_CLOCKS] BSP_PLACE_IN_SECTION(
    BSP_SECTION_NOINIT);

#if BSP_TZ_SECURE_BUILD

/* Callback used to notify the nonsecure project that the clock settings have
 * changed. */
...
static void r_bsp_clock_update_callback_call(
...
/* Initialize the callback, callback memory and invoke the callback to ensure
 * the nonsecure project has the correct clock settings. */
...
static void BSP_CMSE_NONSECURE_CALL
g_bsp_clock_update_callback(bsp_clock_update_callback_args_t *p_callback_args)
#elif defined(__GNUC__)

static BSP_CMSE_NONSECURE_CALL void g_bsp_clock_update_callback(
    bsp_clock_update_callback_args_t *p_callback_args)
#endif
...
/* List of MSTP bits that must be set before entering low power modes or
 * changing SCKDIVCR. */
static const uint8_t g_bsp_prv_power_change_mstp_data[][2] =
    BSP_FEATURE_LPM_CHANGE_MSTP_ARRAY;

static const uint8_t g_bsp_prv_power_change_mstp_length =
    sizeof(g_bsp_prv_power_change_mstp_data) /
...
/***********************************************************************************************************************
 * Changes the operating speed in OPCCR.  Assumes the LPM registers are unlocked
 *in PRCR and cache is off.
 *
 * @param[in]  operating_mode  Desired operating mode, must be one of the
 *BSP_PRV_OPERATING_MODE_* macros, cannot be BSP_PRV_OPERATING_MODE_SUBOSC_SPEED
 **********************************************************************************************************************/
static void bsp_prv_operating_mode_opccr_set(uint8_t operating_mode) {
...
/***********************************************************************************************************************
 * Changes the operating speed mode.  Assumes the LPM registers are unlocked in
 *PRCR and cache is off.
 *
 * @param[in]  operating_mode  Desired operating mode, must be one of the
 *BSP_PRV_OPERATING_MODE_* macros
 **********************************************************************************************************************/
void bsp_prv_operating_mode_set(uint8_t operating_mode) {
  ...
  if ((operating_mode > BSP_PRV_OPERATING_MODE_MIDDLE_SPEED) &&
      (R_SYSTEM->DCDCCTL & R_SYSTEM_DCDCCTL_DCDCON_Msk)) {
    /* LDO boost must be used if entering subclock speed mode (see RA2L1 User's
     * Manual (R01UH0853EJ0100) Section 10.5.1 (5) Switching from
     * High-speed/Middle-speed mode in DCDC power mode to Subosc-speed mode or
     * Software Standby mode). */
    power_mode = R_BSP_PowerModeSet(
        (BSP_PRV_OPERATING_MODE_SUBOSC_SPEED == operating_mode)
...
#endif
  {
    ...
    /* Wait for transition to complete. Check the entire register here since it
     * should be set to 0 at this point. Checking the entire register is
     * slightly more efficient. This will also hang the program if the LPM
...
/***********************************************************************************************************************
 * Updates the operating frequency of the specified PLL and all its output
 *channels.
 *
 * @param[in] clock                    PLL being configured
 * @param[in] p_pll_hz                 Array of values of the new PLL output
 *clock frequencies
 **********************************************************************************************************************/
...
void SystemCoreClockUpdate(void) {
    ...
    (BSP_TZ_SECURE_BUILD || BSP_TZ_NONSECURE_BUILD) && \
    BSP_FEATURE_TZ_VERSION == 2
...
#if !BSP_FEATURE_CGC_HAS_CPUCLK
  uint32_t ick = (FSP_STYPE3_REG32_READ(R_SYSTEM->SCKDIVCR, secure) &
                  R_SYSTEM_SCKDIVCR_ICK_Msk) >>
                 R_SYSTEM_SCKDIVCR_ICK_Pos;
  SystemCoreClock = g_clock_freq[clock_index] >> ick;
#else
  uint8_t cpuck = (FSP_STYPE3_REG8_READ(R_SYSTEM->SCKDIVCR2, secure) &
                   R_SYSTEM_SCKDIVCR2_CPUCK_Msk) >>
...
#if BSP_CLOCK_CFG_MAIN_OSC_POPULATED
  SystemCoreClock =
      g_clock_freq[BSP_CLOCKS_SOURCE_CLOCK_MAIN_OSC] >> R_SYSTEM->MOSCDIV;
#endif
  if (BSP_CLOCKS_SOURCE_CLOCK_FSUB == R_SYSTEM->ICLKSCR_b.CKST) {
#if BSP_CLOCK_CFG_SUBCLOCK_POPULATED
    SystemCoreClock = R_SYSTEM->FSUBSCR
                          ? g_clock_freq[BSP_CLOCKS_SOURCE_CLOCK_LOCO]
...
uint32_t bsp_prv_power_change_mstp_set(void) {
  ...
  for (uint32_t i = 0U; i < g_bsp_prv_power_change_mstp_length; i++) {
    /* Get the MSTP register index and the bit to test from the MCU specific
     * array. */
    ...
    /* This loop takes over 250 ns (30 cycles at 120 MHz) between 2 consecutive
     * bits being set. It was measured at 58 cycles for default IAR build
     * configurations and 59 cycles for default GCC build configurations. */
  }

  /* The time between setting last MSTP bit and setting SCKDIVCR takes over 750
   * ns (90 cycles at 120 MHz). It was measured at 96 cycles for default IAR
   * build configurations and 102 cycles for default GCC build configurations.
   */
...
void bsp_prv_power_change_mstp_clear(uint32_t mstp_clear_bitmask) {
  /* The time between setting SCKDIVCR and clearing the first MSTP bit takes
   * over 250 ns (30 cycles at 120 MHz). It was measured at 38 cycles for
   * default IAR build configurations and 68 cycles for default GCC build
   * configurations. */
  ...
  for (uint32_t i = 0U; i < g_bsp_prv_power_change_mstp_length; i++) {
    ...
    /* This loop takes over 250 ns (30 cycles at 120 MHz) between 2 consecutive
     * bits being cleared. It was measured at 44 cycles for default IAR build
     * configurations and 53 cycles for default GCC build configurations. */
  }
}
...
void prv_clock_dividers_set(uint32_t sckdivcr, uint8_t sckdivcr2) {
  ...
  uint32_t current_iclk_div =
      BSP_PRV_SCKDIVCR_DIV_VALUE(R_SYSTEM->SCKDIVCR_b.ICK);

  if (requested_iclk_div >= current_iclk_div) {
    /* If the requested ICLK divider is greater than or equal to the current
     * ICLK divider, then writing to SCKDIVCR first will always satisfy the
     * constraint: CPUCLK frequency >= ICLK frequency. */
    R_SYSTEM->SCKDIVCR = sckdivcr;
    R_SYSTEM->SCKDIVCR2 = sckdivcr2;
  } else {
    /* If the requested ICLK divider is less than the current ICLK divider, then
     * writing to SCKDIVCR2 first will always satisfy the constraint: CPUCLK
     * frequency >= ICLK frequency. */
    R_SYSTEM->SCKDIVCR2 = sckdivcr2;
    R_SYSTEM->SCKDIVCR = sckdivcr;
  }
...
void bsp_prv_clock_set(uint32_t clock, uint32_t sckdivcr, uint8_t sckdivcr2) {
  ...
  /* Set MSTP bits as required by the hardware manual. This is done first to
   * ensure the 750 ns delay required after increasing any division ratio in
   * SCKDIVCR is met. */
  ...
  uint32_t iclk_div =
      (sckdivcr >> FSP_PRIV_CLOCK_ICLK) & FSP_PRV_SCKDIVCR_DIV_MASK;
  uint32_t iclk_freq_hz_post_change =
      g_clock_freq[clock] / BSP_PRV_SCKDIVCR_DIV_VALUE(iclk_div);
#if BSP_FEATURE_CGC_HAS_CPUCLK
  uint32_t cpuclk_div = sckdivcr2 & R_SYSTEM_SCKDIVCR2_CPUCK_Msk;
  uint32_t clock_freq_hz_post_change =
      g_clock_freq[clock] / BSP_PRV_SCKDIVCR_DIV_VALUE(cpuclk_div);
#else
  uint32_t clock_freq_hz_post_change = iclk_freq_hz_post_change;
#endif

  /* Adjust the MCU specific wait state right before the system clock is set, if
   * the system clock frequency to be set is higher than before. */
  uint8_t new_rom_wait_state =
      bsp_clock_set_prechange(iclk_freq_hz_post_change);
  ...
  if (g_clock_freq[clock] >= g_clock_freq[R_SYSTEM->SCKSCR]) {
    ...
    if ((clock_freq_hz_post_change > SystemCoreClock) &&
        ((clock_freq_hz_post_change - SystemCoreClock) >
      ...
      if (iclk_div == sckdivcr2) {
        ...
        R_SYSTEM->SCKDIVCR = sckdivcr;
        if (cpuclk_div == BSP_CLOCKS_SYS_CLOCK_DIV_1) {
          /* Determine what the other dividers are using and stay aligned with
           * that. */
          R_SYSTEM->SCKDIVCR2 = (iclk_div & 0x8) ? BSP_CLOCKS_SYS_CLOCK_DIV_3
                                                 : BSP_CLOCKS_SYS_CLOCK_DIV_2;
        } else {
    ...
    if (post_div_set_delay) {
      /* Update the CMSIS core clock variable so that it reflects the new ICLK
       * frequency. */
  ...
  /* Switching to a slower source clock. */
    ...
    if ((SystemCoreClock > clock_freq_hz_post_change) &&
      ...
      /* Must first step CPUCLK down by factor of 2 or 3 if it is currently
       * above threshold. */
      if (R_SYSTEM->SCKDIVCR2 ==
          ((current_sckdivcr >> R_SYSTEM_SCKDIVCR_ICK_Pos) & 0xF)) {
        /* If ICLK and CPUCLK have same divider currently, move ICLK down 1
         * notch first. */
        uint32_t current_iclk_div =
            (current_sckdivcr >> R_SYSTEM_SCKDIVCR_ICK_Pos) & 0xF;
        uint32_t new_div = current_iclk_div + 1;
        if (current_iclk_div == 0) {
          /* Align with already selected divider for PCLKA because it must have
           * one > 1 already. */
          new_div = (current_sckdivcr & (0x8 << R_SYSTEM_SCKDIVCR_PCKA_Pos))
                        ? BSP_CLOCKS_SYS_CLOCK_DIV_3
                        : BSP_CLOCKS_SYS_CLOCK_DIV_2;
        }
  ...
  if (NULL != g_bsp_clock_update_callback) {
    ...
    /* Call the callback. */
    r_bsp_clock_update_callback_call(g_bsp_clock_update_callback,
                                     &callback_args);
  }
#endif

  /* Adjust the MCU specific wait state soon after the system clock is set, if
   * the system clock frequency to be set is lower than previous. */
  bsp_clock_set_postchange(SystemCoreClock, new_rom_wait_state);

#if BSP_FEATURE_LPM_CHANGE_MSTP_REQUIRED

  /* Clear MSTP bits as required by the hardware manual. This is done last to
   * ensure the 250 ns delay required after decreasing any division ratio in
   * SCKDIVCR is met. */
...
bool bsp_prv_clock_prepare_pre_sleep(void) {
  ...
  if (SystemCoreClock > BSP_MAX_CLOCK_CHANGE_THRESHOLD) {
    /* Reduce speed of CPUCLK to /2 or /3 of current, select which ones based on
     * what ICLK divider is. */
    R_SYSTEM->SCKDIVCR2 =
        (R_SYSTEM->SCKDIVCR & (0x8 << R_SYSTEM_SCKDIVCR_ICK_Pos))
            ? BSP_CLOCKS_SYS_CLOCK_DIV_3
            : BSP_CLOCKS_SYS_CLOCK_DIV_2;
    cpuclk_slowed = true;
    SystemCoreClockUpdate();
...
void bsp_prv_clock_prepare_post_sleep(bool cpuclk_slowed) {
  /* Set CPUCLK back to original speed here if it was slowed down before
   * sleeping (dropped to below 240MHz) Add delays as described in
   * Section 10.7.10 of RA8M1 manual R01UH0994EJ0100 */
  R_BSP_SoftwareDelay(BSP_CFG_CLOCK_SETTLING_DELAY_US,
                      BSP_DELAY_UNITS_MICROSECONDS);
  if (cpuclk_slowed) {
    /* Set divider of CPUCLK back to /1. This is the only possible value for it
     * to have been over 240MHz before sleeping. */
...
uint32_t bsp_prv_clock_source_get(void) {
  /*
   * | System clock source | FOCOSCR.CKSEL | FMAINSCR.CKSEL | FSUBSCR.CKSEL |
   * ICLKSCR.CKSEL | | ------------------- | ------------- | -------------- |
   * ------------- | ------------- | | HOCO                | 0U            | 0U
   * | x             | 0U            | | MOCO                | 1U            |
   * 0U             | x             | 0U            | | MOSC                | x
  ...
  if (BSP_CLOCKS_SOURCE_CLOCK_FSUB == R_SYSTEM->ICLKSCR_b.CKST) {
    clock = R_SYSTEM->FSUBSCR ? BSP_CLOCKS_SOURCE_CLOCK_LOCO
                              : BSP_CLOCKS_SOURCE_CLOCK_SUBCLOCK;
  } else if (BSP_CLOCKS_FMAIN_SOURCE_CLOCK_FOCO == R_SYSTEM->FMAINSCR_b.CKST) {
    clock = R_SYSTEM->FOCOSCR_b.CKST ? BSP_CLOCKS_SOURCE_CLOCK_MOCO
                                     : BSP_CLOCKS_SOURCE_CLOCK_HOCO;
  } else {
    /* Do nothing. */
  }
...
void bsp_prv_clock_set(uint32_t clock, uint8_t hocodiv, uint8_t mocodiv,
  ...
  FSP_HARDWARE_REGISTER_WAIT(R_SYSTEM->ICLKSCR_b.CKST,
                             BSP_CLOCKS_SOURCE_CLOCK_FMAIN);

  if ((BSP_CLOCKS_SOURCE_CLOCK_HOCO == clock) ||
      (BSP_CLOCKS_SOURCE_CLOCK_MOCO == clock)) {
    R_SYSTEM->FMAINSCR_b.CKSEL = BSP_CLOCKS_FMAIN_SOURCE_CLOCK_FOCO;
    FSP_HARDWARE_REGISTER_WAIT(R_SYSTEM->FMAINSCR_b.CKST,
                               BSP_CLOCKS_FMAIN_SOURCE_CLOCK_FOCO);

    if (BSP_CLOCKS_SOURCE_CLOCK_HOCO == clock) {
      R_SYSTEM->FOCOSCR_b.CKSEL = BSP_CLOCKS_FOCO_SOURCE_CLOCK_HOCO;
      FSP_HARDWARE_REGISTER_WAIT(R_SYSTEM->FOCOSCR_b.CKST,
                                 BSP_CLOCKS_FOCO_SOURCE_CLOCK_HOCO);

      /* Due to register access restrictions (see 8.6.1 Register Access), only
       * set the HOCODIV when system clock source is HOCO */
      R_SYSTEM->HOCODIV = hocodiv;
    } else {
      R_SYSTEM->FOCOSCR_b.CKSEL = BSP_CLOCKS_FOCO_SOURCE_CLOCK_MOCO;
      FSP_HARDWARE_REGISTER_WAIT(R_SYSTEM->FOCOSCR_b.CKST,
                                 BSP_CLOCKS_FOCO_SOURCE_CLOCK_MOCO);
    }
  }
    ...
    FSP_HARDWARE_REGISTER_WAIT(R_SYSTEM->FMAINSCR_b.CKST,
                               BSP_CLOCKS_FMAIN_SOURCE_CLOCK_MAIN_OSC);
  }
#endif
  else {
    ...
    FSP_HARDWARE_REGISTER_WAIT(R_SYSTEM->ICLKSCR_b.CKST,
                               BSP_CLOCKS_SOURCE_CLOCK_FSUB);
  }

  R_SYSTEM->MOCODIV = mocodiv;
  R_SYSTEM->MOSCDIV = moscdiv;

  /* Clock is now at requested frequency. Update the CMSIS core clock variable
   * so that it reflects the new ICLK frequency.*/
  SystemCoreClockUpdate();
}
...
void bsp_prv_clkout_clock_set(void) {
  ...
  /* Due to register access restrictions (see 8.6.1 Register Access), change the
   * ICLKSCR.CKSEL = 1 (ICLK = FMAIN) before configuration */
  R_SYSTEM->ICLKSCR_b.CKSEL = BSP_CLOCKS_SOURCE_CLOCK_FMAIN;
  FSP_HARDWARE_REGISTER_WAIT(R_SYSTEM->ICLKSCR_b.CKST,
                             BSP_CLOCKS_SOURCE_CLOCK_FMAIN);
...
static void bsp_prv_clock_set_hard_reset(void) {
  ...
  /* The MCU must be in high speed mode to set wait states to 2. High speed mode
   * is the default out of reset. */
  ...
  /* In order to avoid a system clock (momentarily) higher than expected, the
   * order of switching the clock and dividers must be so that the frequency of
   * the clock goes lower, instead of higher, before being correct. */

  /* MOCO is the source clock after reset. If the new source clock is faster
   * than the current source clock, then set the clock dividers before switching
   * to the new source clock. */
#if BSP_MOCO_FREQ_HZ <= BSP_STARTUP_SOURCE_CLOCK_HZ
#if BSP_FEATURE_CGC_HAS_CPUCLK
#if BSP_CFG_CLOCK_SETTLING_DELAY_ENABLE && \
    (BSP_STARTUP_CPUCLK_HZ >= BSP_MAX_CLOCK_CHANGE_THRESHOLD)

/* If the requested ICLK divider is greater than or equal to the current ICLK
 * divider, then writing to SCKDIVCR first will always satisfy the constraint:
 * CPUCLK frequency >= ICLK frequency. */
  ...
  R_SYSTEM->SCKDIVCR =
      (BSP_PRV_STARTUP_SCKDIVCR & ~(R_SYSTEM_SCKDIVCR_ICK_Msk)) |
  ...
  R_SYSTEM->SCKDIVCR2 = (BSP_CFG_ICLK_DIV & 0x8) ? BSP_CLOCKS_SYS_CLOCK_DIV_3
                                                 : BSP_CLOCKS_SYS_CLOCK_DIV_2;
#else
  ...
  R_BSP_SoftwareDelay(BSP_CFG_CLOCK_SETTLING_DELAY_US,
                      BSP_DELAY_UNITS_MICROSECONDS);
#else
#if BSP_PRV_ICLK_DIV_VALUE >= \
    BSP_PRV_SCKDIVCR_DIV_VALUE(BSP_FEATURE_CGC_ICLK_DIV_RESET)
  ...
  /* MOCO is the source clock after reset. If the new source clock is slower
   * than the current source clock, then set the clock dividers after switching
   * to the new source clock. */
#if BSP_MOCO_FREQ_HZ > BSP_STARTUP_SOURCE_CLOCK_HZ
#if BSP_FEATURE_CGC_HAS_CPUCLK
#if BSP_PRV_ICLK_DIV_VALUE >= \
  ...
  /* Execute data memory barrier before and after setting the wait states, See
   * Section 50.4.2 in the RA8M1 manual R01UH0994EJ0100 */
  __DMB();
  R_SRAM->SRAMWTSC = BSP_PRV_SRAMWTSC_WAIT_CYCLES_DISABLE;
  __DMB();
...
#endif
{
  ...
  /* If the CGC is secure and this is a non secure project, register a callback
   * for getting clock settings. */
...
#if BSP_PRV_PLL_SUPPORTED && \
    BSP_CLOCKS_SOURCE_CLOCK_PLL != BSP_CFG_CLOCK_SOURCE && BSP_PRV_PLL_USED
...
/*
 * If the clock registers are not guaranteed to be set to their value after
 * reset (Ie. the application is executing after a bootloader), then the current
 * state of the registers must be taken into consideration before writing the
 * clock configuration.
 *
 * The HOCO must be stopped in the following situations:
 *  - The application configures the HOCO to be stopped.
 *  - The application enables the FLL, but the HOCO is already running. In order
 * to enable the FLL, the HOCO must be stopped. The PLL must be stopped in the
 * following situations:
 *  - The application configures the PLL to be stopped.
 *  - The application configures settings that are different than the current
 * settings, but the PLL is already running. In order to write new PLL settings,
 * the PLL must be stopped.
 ...
 *  - The application configures settings that are different than the current
 * settings, but the PLL2 is already running. In order to write new PLL2
 * settings, the PLL2 must be stopped.
 *  - The HOCO is the PLL2 source clock and the HOCO is being stopped.
 *
 * If the HOCO or PLL are being used as the system clock source and they need to
 * be stopped, then the system clock source needs to be switched to the default
 * system clock source before the current system clock source is disabled.
 */
void bsp_soft_reset_prepare(void) {
  ...
  /* If the HOCO is already enabled and either the FLL needs to be enabled or
   * the HOCO is not used, then stop the HOCO. */
  ...
  if (0 == R_SYSTEM->PLLCR) {
    /*
     * If any of the following conditions are true, then the PLL needs to be
     * stopped:
     * - The PLL is not used
     * - The PLL settings need to be changed
     * - The HOCO is selected as the PLL clock source and the HOCO needs to be
     * stopped
     *   - Note that PLL type 2 does not support running off of the HOCO
     */
#if BSP_PRV_PLL_USED
#if 3 == BSP_FEATURE_CGC_PLLCCR_TYPE
    if ((BSP_PRV_PLLCCR != R_SYSTEM->PLLCCR) ||
        (BSP_PRV_PLLCCR2 != R_SYSTEM->PLLCCR2) ||
  ...
  if (0 == R_SYSTEM->PLL2CR) {
    /*
     * If any of the following conditions are true, then the PLL2 needs to be
     * stopped:
     * - The PLL2 is not used
     * - The PLL2 settings need to be changed
     * - The HOCO is selected as the PLL2 clock source and the HOCO needs to be
     * stopped
     *   - Note that PLL type 2 does not support running off of the HOCO
     */
#if BSP_PRV_PLL2_USED
#if 3 == BSP_FEATURE_CGC_PLLCCR_TYPE
    if ((BSP_PRV_PLL2CCR != R_SYSTEM->PLL2CCR) ||
        (BSP_PRV_PLL2CCR2 != R_SYSTEM->PLL2CCR2) ||
  ...
  /* Disable the oscillators so that the application can write the new clock
   * configuration. */
...
void bsp_prv_cmc_init(void) {
  /* The CMC register can be written only once after release from the reset
   * state. If clock registers not reset values during startup, assume CMC
   * register has already been set appropriately. */
...
/***********************************************************************************************************************
 * Changes the operating speed in FLMODE.  Assumes the LPM registers are
 *unlocked in PRCR.
 *
 * @param[in]  operating_mode  Desired operating mode, must be one of the
 *BSP_PRV_OPERATING_MODE_* macros, cannot be BSP_PRV_OPERATING_MODE_SUBOSC_SPEED
...
static void bsp_prv_operating_mode_flmode_set(uint8_t operating_mode) {
  if (operating_mode != R_FACI_LP->FLMODE_b.MODE) {
    ...
    if ((BSP_PRV_OPERATING_MODE_MIDDLE_SPEED != operating_mode) &&
      ...
      R_FACI_LP->FLMODE = (uint8_t)(BSP_PRV_OPERATING_MODE_MIDDLE_SPEED
...
void bsp_clock_init(void) {
  ...
  /* Enable the flash cache and don't disable it while running from flash. On
   * these MCUs, the flash cache does not need to be disabled when adjusting the
  ...
  /* Transition to an intermediate clock configuration in order to prepare for
   * writing the new clock configuraiton. */
  ...
  /* Update the main oscillator drive, source, and wait states if the main
   * oscillator is stopped.  If the main oscillator is running, the drive,
   * source, and wait states are assumed to be already set appropriately. */
  if (R_SYSTEM->MOSCCR) {
#if BSP_FEATURE_CGC_REGISTER_SET_B
  ...
  /* HOCO is assumed to be stable because these MCUs also require the HOCO to be
   * stable before changing the operating power control mode. */
#endif
  R_SYSTEM->HOCOWTCR = BSP_FEATURE_CGC_HOCOWTCR_VALUE;
#endif
#else

  /* These MCUs require HOCOWTCR to be set to the maximum value except in snooze
   * mode.  There is no restriction to writing this register. */
  R_SYSTEM->HOCOWTCR = BSP_FEATURE_CGC_HOCOWTCR_VALUE;
#endif
#endif
  ...
  /* Switch to high-speed to prevent any issues with the subsequent clock
   * configurations. */
  ...
  /* HOCO must be running during startup in low voltage mode. If HOCO is not
   * used, turn it off after exiting low voltage mode. */
  R_SYSTEM->HOCOCR = 1U;
#endif
  ...
  R_BSP_SoftwareDelay(BSP_PRV_FLL_STABILIZATION_TIME_US,
                      BSP_DELAY_UNITS_MICROSECONDS);
#endif
  ...
  /* If the MOCO is not running, start it and wait for it to stabilize using a
   * software delay. */
  if (0U != R_SYSTEM->MOCOCR) {
    R_SYSTEM->MOCOCR = 0U;
#if BSP_PRV_STABILIZE_MOCO
    R_BSP_SoftwareDelay(BSP_FEATURE_CGC_MOCO_STABILIZATION_MAX_US,
                        BSP_DELAY_UNITS_MICROSECONDS);
#endif
  }
  ...
  /* If the LOCO is not running, start it and wait for it to stabilize using a
   * software delay. */
...
#endif
  {
     ...
     *  0xFF:  2^18/fx min
     */
    uint8_t mainosc_stable_value =
        (uint8_t)~(BSP_PRV_OSTC_OFFSET >> BSP_CLOCK_CFG_MAIN_OSC_WAIT);
  ...
  /* Start clocks that require other clocks. At this point, all dependent clocks
   * are running and stable if needed. */
...
#endif
  {
    ...
    /* This loop is provided to ensure at least 1 us passes between setting
     * PLLMUL and clearing PLLSTP on some MCUs (see PLLSTP notes in
     * Section 8.2.4 "PLL Control Register (PLLCR)" of the RA4M1 manual
     * R01UH0887EJ0100). Five loops are needed here to ensure the most efficient
     * path takes at least 1 us from the setting of PLLMUL to the clearing of
     * PLLSTP. HOCO is the fastest clock we can be using here since PLL cannot
     * be running while setting PLLCCR. */
    bsp_prv_software_delay_loop(
        BSP_DELAY_LOOPS_CALCULATE(BSP_PRV_MAX_HOCO_CYCLES_PER_US));
#endif
...
#endif
#else
  bsp_prv_clock_set(BSP_CFG_CLOCK_SOURCE, BSP_CFG_HOCO_DIV, BSP_CFG_MOCO_DIV,
                    BSP_CFG_XTAL_DIV);
#endif

  /* If the MCU can run in a lower power mode, apply the optimal operating speed
   * mode. */
#if !BSP_CFG_USE_LOW_VOLTAGE_MODE
...
#if defined(BSP_PRV_POWER_USE_DCDC) &&                        \
    (BSP_PRV_POWER_USE_DCDC == BSP_PRV_POWER_DCDC_STARTUP) && \
    (BSP_PRV_STARTUP_OPERATING_MODE <= BSP_PRV_OPERATING_MODE_MIDDLE_SPEED)

  /* Start DCDC as part of BSP startup when configured (BSP_CFG_DCDC_ENABLE ==
   * 2). */
...
#else
  uint8_t ckocr =
      BSP_CFG_CLKOUT_SOURCE | (BSP_CFG_CLKOUT_DIV << BSP_PRV_CKOCR_CKODIV_BIT);
  ...
  /* If there is a REQ bit in USBCKCR, then follow sequence from section 8.2.29
   * in RA6M4 hardware manual R01UH0890EJ0050. */
  ...
  /* Set the OCTASPI clock if it exists on the MCU (See section 8.2.30 of the
   * RA6M4 hardware manual R01UH0890EJ0050). */
#if BSP_FEATURE_BSP_HAS_OCTASPI_CLOCK && \
    BSP_CFG_OCTA_SOURCE != BSP_CLOCKS_CLOCK_DISABLED
  ...
  bsp_peripheral_clock_set(&R_SYSTEM->CANFDCKCR, &R_SYSTEM->CANFDCKDIVCR,
                           BSP_CFG_CANFDCLK_DIV, BSP_CFG_CANFDCLK_SOURCE);
#endif

  /* Set the SCISPI clock if it exists on the MCU */
#if BSP_FEATURE_BSP_HAS_SCISPI_CLOCK && \
    (BSP_CFG_SCISPICLK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)
  bsp_peripheral_clock_set(&R_SYSTEM->SCISPICKCR, &R_SYSTEM->SCISPICKDIVCR,
                           BSP_CFG_SCISPICLK_DIV, BSP_CFG_SCISPICLK_SOURCE);
#endif

  /* Set the SCI clock if it exists on the MCU */
#if BSP_FEATURE_BSP_HAS_SCI_CLOCK && \
    (BSP_CFG_SCICLK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)
  bsp_peripheral_clock_set(&R_SYSTEM->SCICKCR, &R_SYSTEM->SCICKDIVCR,
                           BSP_CFG_SCICLK_DIV, BSP_CFG_SCICLK_SOURCE);
#endif

  /* Set the SPI clock if it exists on the MCU */
#if BSP_FEATURE_BSP_HAS_SPI_CLOCK && \
    (BSP_CFG_SPICLK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)
  bsp_peripheral_clock_set(&R_SYSTEM->SPICKCR, &R_SYSTEM->SPICKDIVCR,
                           BSP_CFG_SPICLK_DIV, BSP_CFG_SPICLK_SOURCE);
#endif

  /* Set the GPT clock if it exists on the MCU */
#if BSP_FEATURE_BSP_HAS_GPT_CLOCK && \
    (BSP_CFG_GPTCLK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)
  bsp_peripheral_clock_set(&R_SYSTEM->GPTCKCR, &R_SYSTEM->GPTCKDIVCR,
                           BSP_CFG_GPTCLK_DIV, BSP_CFG_GPTCLK_SOURCE);
#endif

  /* Set the IIC clock if it exists on the MCU */
#if BSP_FEATURE_BSP_HAS_IIC_CLOCK && \
    (BSP_CFG_IICCLK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)
  bsp_peripheral_clock_set(&R_SYSTEM->IICCKCR, &R_SYSTEM->IICCKDIVCR,
                           BSP_CFG_IICCLK_DIV, BSP_CFG_IICCLK_SOURCE);
#endif

  /* Set the CEC clock if it exists on the MCU */
#if BSP_FEATURE_BSP_HAS_CEC_CLOCK && \
    (BSP_CFG_CECCLK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)
  bsp_peripheral_clock_set(&R_SYSTEM->CECCKCR, &R_SYSTEM->CECCKDIVCR,
                           BSP_CFG_CECCLK_DIV, BSP_CFG_CECCLK_SOURCE);
#endif

  /* Set the I3C clock if it exists on the MCU */
#if BSP_FEATURE_BSP_HAS_I3C_CLOCK && \
    (BSP_CFG_I3CCLK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)
  bsp_peripheral_clock_set(&R_SYSTEM->I3CCKCR, &R_SYSTEM->I3CCKDIVCR,
                           BSP_CFG_I3CCLK_DIV, BSP_CFG_I3CCLK_SOURCE);
#endif

  /* Set the LCD clock if it exists on the MCU */
#if BSP_FEATURE_BSP_HAS_LCD_CLOCK && \
    (BSP_CFG_LCDCLK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)
  bsp_peripheral_clock_set(&R_SYSTEM->LCDCKCR, &R_SYSTEM->LCDCKDIVCR,
                           BSP_CFG_LCDCLK_DIV, BSP_CFG_LCDCLK_SOURCE);
#endif

  /* Set the USB-HS clock if it exists on the MCU */
#if BSP_FEATURE_BSP_HAS_USB60_CLOCK_REQ && \
    (BSP_CFG_U60CK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)
  bsp_peripheral_clock_set(&R_SYSTEM->USB60CKCR, &R_SYSTEM->USB60CKDIVCR,
                           BSP_CFG_U60CK_DIV, BSP_CFG_U60CK_SOURCE);
#endif

  /* Set the SDADC clock if it exists on the MCU. */
#if BSP_FEATURE_BSP_HAS_SDADC_CLOCK && \
    (BSP_CFG_SDADC_CLOCK_SOURCE != BSP_CLOCKS_CLOCK_DISABLED)
  ...
  /* SDADC isn't controlled like the other peripheral clocks so we cannot use
   * the generic setter. */
...
static void bsp_peripheral_clock_set(volatile uint8_t *p_clk_ctrl_reg,
  ...
  *p_clk_ctrl_reg |= (uint8_t)BSP_PRV_PERIPHERAL_CLK_REQ_BIT_MASK;
  ...
  FSP_HARDWARE_REGISTER_WAIT(
      (uint8_t)((*p_clk_ctrl_reg & BSP_PRV_PERIPHERAL_CLK_RDY_BIT_MASK) >>
                BSP_PRV_PERIPHERAL_CLK_RDY_BIT_POS),
      1U);

  /* Select the peripheral clock divisor and source. */
  *p_clk_div_reg = peripheral_clk_div;
  *p_clk_ctrl_reg = peripheral_clk_source |
                    BSP_PRV_PERIPHERAL_CLK_REQ_BIT_MASK |
...
static uint8_t bsp_clock_set_prechange(uint32_t requested_freq_hz) {
  ...
  if (requested_freq_hz <= BSP_FEATURE_BSP_SYS_CLOCK_FREQ_ONE_ROM_WAITS) {
    ...
    new_rom_wait_state = BSP_PRV_ROM_ONE_WAIT_CYCLES;
             ...
             BSP_FEATURE_BSP_SYS_CLOCK_FREQ_THREE_ROM_WAITS) {
    new_rom_wait_state = BSP_PRV_ROM_TWO_WAIT_CYCLES;
  } else {
  ...
  if (requested_freq_hz <= BSP_FEATURE_BSP_SYS_CLOCK_FREQ_ONE_ROM_WAITS) {
    ...
    new_rom_wait_state = BSP_PRV_ROM_TWO_WAIT_CYCLES;
             ...
             BSP_FEATURE_BSP_SYS_CLOCK_FREQ_FOUR_ROM_WAITS) {
    new_rom_wait_state = BSP_PRV_ROM_THREE_WAIT_CYCLES;
  } else {
  ...
  if (requested_freq_hz <= BSP_FEATURE_BSP_SYS_CLOCK_FREQ_ONE_ROM_WAITS) {
    ...
    new_rom_wait_state = BSP_PRV_ROM_THREE_WAIT_CYCLES;
             ...
             BSP_FEATURE_BSP_SYS_CLOCK_FREQ_FIVE_ROM_WAITS) {
    new_rom_wait_state = BSP_PRV_ROM_FOUR_WAIT_CYCLES;
  } else {
  ...
  /* If more wait states are required after the change, then set the wait states
  ...
  if (requested_freq_hz > BSP_PRV_FLDWAITR_MAX_ONE_WAIT_FREQ) {
    /* The MCU must be in high speed mode to set wait states to 2. The MCU
     * should already be in high speed mode as a precondition to
     * bsp_prv_clock_set. */
    BSP_PRV_FLDWAITR_REG_ACCESS = BSP_PRV_FLDWAITR_TWO_WAIT_CYCLES;
  }
#endif
...
static void bsp_prv_sosc_init(void) {
  ...
  /* If sub-clock is used as system clock source or HOCO FLL source, wait for
   * VRTC-domain become valid */
...
#if !BSP_FEATURE_CGC_REGISTER_SET_B
    if (R_SYSTEM->SOSCCR ||
        (BSP_CLOCK_CFG_SUBCLOCK_DRIVE != R_SYSTEM->SOMCR_b.SODRV)) {
      /* If Sub-Clock Oscillator is started at reset, stop it before configuring
       * the subclock drive. */
      if (0U == R_SYSTEM->SOSCCR) {
        ...
        /* Allow a stop interval of at least 5 SOSC clock cycles before
         * configuring the drive capacity and restarting Sub-Clock Oscillator.
         */
        R_BSP_SoftwareDelay(BSP_PRV_SUBCLOCK_STOP_INTERVAL_US,
                            BSP_DELAY_UNITS_MICROSECONDS);

        /*
         * r01uh0893ej0120-ra4m3 8.2.9 SOSCCR : Sub-Clock Oscillator Control
         * Register: When changing the value of the SOSTP bit, execute
         * subsequent instructions only after reading the bit to check that the
         * value is updated.
         */
        FSP_HARDWARE_REGISTER_WAIT(R_SYSTEM->SOSCCR, 1U);
      }
      ...
      R_SYSTEM->SOMCR =
          ((BSP_CLOCK_CFG_SUBCLOCK_DRIVE << BSP_FEATURE_CGC_SODRV_SHIFT) &
           BSP_FEATURE_CGC_SODRV_MASK);
#else
    if (R_SYSTEM->SOSCCR) {
#endif
      ...
      /* r01uh0893ej0120-ra4m3 8.2.9 SOSCCR : Sub-Clock Oscillator Control
       * Register: After setting the SOSTP bit to 0, use the sub-clock only
       * after the sub-clock oscillation stabilization time has elapsed.
       */
#if (BSP_CLOCKS_SOURCE_CLOCK_SUBCLOCK == BSP_CFG_CLOCK_SOURCE) || \
    (BSP_PRV_HOCO_USE_FLL)
      ...
      /*
       * RA MCUs like RA6M5 requires to use sub-clock after oscillation
       * stabilization time has elapsed on Power-On-Reset. But, POR is not well
       * supported on EK boards, so BSP has to wait on any reset. Please
       * override this function in application if waiting for stabilization is
       * not required.
       */
...
void R_BSP_OctaclkUpdate(bsp_octaclk_settings_t *p_octaclk_setting) {
  ...
  R_SYSTEM->OCTACKCR = (uint8_t)(p_octaclk_setting->source_clock |
                                 R_SYSTEM_OCTACKCR_OCTACKSREQ_Msk);
...
void R_BSP_Init_RTC(void) {
...
#if !BSP_CFG_RTC_USED
#if BSP_PRV_LOCO_USED || \
    (BSP_FEATURE_CGC_HAS_SOSC && BSP_CLOCK_CFG_SUBCLOCK_POPULATED)
#if !BSP_FEATURE_CGC_REGISTER_SET_B
  ...
  /* When the RCR1 register is modified, check that all the bits are updated
   * before proceeding (see section 26.2.17 "RTC Control Register 1 (RCR1)" of
...
fsp_err_t R_BSP_SubclockStatusGet() {
  ...
  FSP_ERROR_RETURN(1U == R_SYSTEM->VRTSR_b.VRTVLD,
