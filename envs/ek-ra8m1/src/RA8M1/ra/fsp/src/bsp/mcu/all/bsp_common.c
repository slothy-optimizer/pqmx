...
#define WEAK_ERROR_ATTRIBUTE \
  __attribute__((weak, alias("fsp_error_log_internal")))
...
/** Prototype of initialization function called before main.  This prototype
 * sets the weak association of this function to an internal example
 * implementation. If this function is defined in the application code, the
...
/** Prototype of function called before errors are returned in FSP code if
 * BSP_CFG_ERROR_LOG is set to 1.  This prototype sets the weak association of
 * this function to an internal example implementation. */

void fsp_error_log(fsp_err_t err, const char *file,
                   int32_t line) WEAK_ERROR_ATTRIBUTE;

void fsp_error_log_internal(fsp_err_t err, const char *file,
                            int32_t line);  /// Default error logger function
...
static BSP_DONT_REMOVE const fsp_pack_version_t g_fsp_version
    BSP_PLACE_IN_SECTION(FSP_SECTION_VERSION) = {
        .version_id_b = {.minor = FSP_VERSION_MINOR,
                         .major = FSP_VERSION_MAJOR,
                         .build = FSP_VERSION_BUILD,
                         .patch = FSP_VERSION_PATCH}};

/* Public FSP version name. */
static BSP_DONT_REMOVE const uint8_t
    g_fsp_version_string[] BSP_PLACE_IN_SECTION(FSP_SECTION_VERSION) =
        FSP_VERSION_STRING;

/* Unique FSP version ID. */
static BSP_DONT_REMOVE const uint8_t
    g_fsp_version_build_string[] BSP_PLACE_IN_SECTION(FSP_SECTION_VERSION) =
...
R_BSP_NSC_STYPE3_RegU8Read(uint8_t volatile const *p_reg) {
  uint8_t volatile *p_reg_s =
      (uint8_t volatile *)((uint32_t)p_reg & ~BSP_FEATURE_TZ_NS_OFFSET);

  /* Table of secure registers that may be read from the non-secure application.
   */
  static const uint32_t valid_addresses[] = {
      (uint32_t)&R_SYSTEM->SCKDIVCR2,  (uint32_t)&R_SYSTEM->SCKSCR,
      (uint32_t)&R_SYSTEM->SPICKDIVCR, (uint32_t)&R_SYSTEM->SPICKCR,
      (uint32_t)&R_SYSTEM->SCICKDIVCR, (uint32_t)&R_SYSTEM->SCICKCR,
      (uint32_t)&R_SYSTEM->CANFDCKCR,  (uint32_t)&R_SYSTEM->PLLCR,
      (uint32_t)&R_SYSTEM->PLL2CR,     (uint32_t)&R_SYSTEM->MOCOCR,
      (uint32_t)&R_SYSTEM->OPCCR,
  };

  if (bsp_valid_register_check(
          (uint32_t)p_reg_s, valid_addresses,
          sizeof(valid_addresses) / sizeof(valid_addresses[0]))) {
    return *p_reg_s;
  }

  /* Generate a trustzone access violation by accessing the non-secure aliased
   * address. */
...
R_BSP_NSC_STYPE3_RegU16Read(uint16_t volatile const *p_reg) {
  uint16_t volatile *p_reg_s =
      (uint16_t volatile *)((uint32_t)p_reg & ~BSP_FEATURE_TZ_NS_OFFSET);
...
R_BSP_NSC_STYPE3_RegU32Read(uint32_t volatile const *p_reg) {
  uint32_t volatile *p_reg_s =
      (uint32_t volatile *)((uint32_t)p_reg & ~BSP_FEATURE_TZ_NS_OFFSET);
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
__attribute__((weak, noreturn)) void __aeabi_assert(const char *expr,
                                                    const char *file,
                                                    int line) {
...
/* The default assert implementation for GCC brings in printing/formatting code.
 * FSP overrides the default assert behavior to reduce code size. */
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
BSP_WEAK_REFERENCE void __assert_func(const char *file, int line,
