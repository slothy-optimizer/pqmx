...
/** This function is called before returning an error code. To stop on a runtime
 * error, define fsp_error_log in user code and do required debugging
 * (breakpoints, stack dump, etc) in this function.*/
#if (1 == BSP_CFG_ERROR_LOG)

#ifndef FSP_ERROR_LOG
#define FSP_ERROR_LOG(err) fsp_error_log((err), __FILE__, __LINE__);
#endif
#else
...
/** Default assertion calls ::FSP_ERROR_RETURN if condition "a" is false. Used
 * to identify incorrect use of API's in FSP functions. */
#if (3 == BSP_CFG_ASSERT)
#define FSP_ASSERT(a)
...
/** All FSP error codes are returned using this macro. Calls ::FSP_ERROR_LOG
 * function if condition "a" is false. Used to identify runtime errors in FSP
 * functions. */
...
/* Function-like macro used to wait for a condition to be met, most often used
 * to wait for hardware register updates. This macro can be redefined to add a
 ...
 *
 * The macros __CORE__ , __ARM7EM__ and __ARM_ARCH_8M_BASE__ are undefined for
 * GCC, but defined(__IAR_SYSTEMS_ICC__) is false for GCC, so the left half of
 * the || expression evaluates to false for GCC regardless of the values of
 * these macros. */

#if (defined(__IAR_SYSTEMS_ICC__) &&                                      \
     ((__CORE__ == __ARM7EM__) || (__CORE__ == __ARM_ARCH_8M_BASE__))) || \
...
/* These macros abstract methods to save and restore the interrupt state for
...
#define FSP_CRITICAL_SECTION_IRQ_MASK_SET                \
  ((uint8_t)(BSP_CFG_IRQ_MASK_LEVEL_FOR_CRITICAL_SECTION \
             << (8U - __NVIC_PRIO_BITS)))
#endif

/** This macro temporarily saves the current interrupt state and disables
 * interrupts. */
...
/** This macro restores the previously saved interrupt state, reenabling
 * interrupts. */
#ifndef FSP_CRITICAL_SECTION_EXIT
#define FSP_CRITICAL_SECTION_EXIT FSP_CRITICAL_SECTION_SET_STATE(old_mask_level)
#endif

/* Number of Cortex processor exceptions, used as an offset from XPSR value for
 * the IRQn_Type macro. */
...
/* Private definition used in bsp_clocks and R_FSP_SystemClockHzGet. Each
 * bitfield in SCKDIVCR is up to 4 bits wide. */
...
BSP_CMSE_NONSECURE_ENTRY uint8_t
R_BSP_NSC_STYPE3_RegU8Read(uint8_t volatile const *p_reg);
BSP_CMSE_NONSECURE_ENTRY uint16_t
R_BSP_NSC_STYPE3_RegU16Read(uint16_t volatile const *p_reg);
BSP_CMSE_NONSECURE_ENTRY uint32_t
R_BSP_NSC_STYPE3_RegU32Read(uint32_t volatile const *p_reg);
...
/*
 * If the STYPE3 register's security attribution is set to secure, the
 * non-secure application must read the register from the secure application
 * using the provided non-secure callable functions.
 */
#define FSP_STYPE3_REG8_READ(X, S) \
  (!(S) ? X : (R_BSP_NSC_STYPE3_RegU8Read((uint8_t const volatile *)&X)))
#define FSP_STYPE3_REG16_READ(X, S) \
  (!(S) ? X : (R_BSP_NSC_STYPE3_RegU16Read((uint16_t const volatile *)&X)))
#define FSP_STYPE3_REG32_READ(X, S) \
  (!(S) ? X : (R_BSP_NSC_STYPE3_RegU32Read((uint32_t const volatile *)&X)))
#elif BSP_FEATURE_TZ_HAS_TRUSTZONE && BSP_TZ_SECURE_BUILD && \
    BSP_FEATURE_TZ_VERSION == 2
...
R_BSP_S_STYPE3_RegU8Read(uint8_t volatile const *p_reg) {
  p_reg =
      (uint8_t volatile const *)((uint32_t)p_reg | BSP_FEATURE_TZ_NS_OFFSET);
...
R_BSP_S_STYPE3_RegU16Read(uint16_t volatile const *p_reg) {
  p_reg =
      (uint16_t volatile const *)((uint32_t)p_reg | BSP_FEATURE_TZ_NS_OFFSET);
...
R_BSP_S_STYPE3_RegU32Read(uint32_t volatile const *p_reg) {
  p_reg =
      (uint32_t volatile const *)((uint32_t)p_reg | BSP_FEATURE_TZ_NS_OFFSET);

  return *p_reg;
}

/*
 * If the STYPE3 register's security attribution is set to non-secure, the
 * secure application must read the register using the non-secure aliased
 * address.
 */
#define FSP_STYPE3_REG8_READ(X, S) \
  ((S) ? (X) : R_BSP_S_STYPE3_RegU8Read((uint8_t const volatile *)&X))
#define FSP_STYPE3_REG16_READ(X, S) \
  ((S) ? (X) : R_BSP_S_STYPE3_RegU16Read((uint16_t const volatile *)&X))
#define FSP_STYPE3_REG32_READ(X, S) \
  ((S) ? (X) : R_BSP_S_STYPE3_RegU32Read((uint32_t const volatile *)&X))
#else
...
typedef enum e_bsp_warm_start_event {
  BSP_WARM_START_RESET = 0,   ///< Called almost immediately after reset. No C
                              ///< runtime environment, clocks, or IRQs.
  BSP_WARM_START_POST_CLOCK,  ///< Called after clock initialization. No C
                              ///< runtime environment or IRQs.
  BSP_WARM_START_POST_C  ///< Called after clocks and C runtime environment have
                         ///< been set up
} bsp_warm_start_event_t;

/* Private enum used in R_FSP_SystemClockHzGet.  Maps clock name to base bit in
 * SCKDIVCR. */
...
/* Private enum used in R_FSP_SciSpiClockHzGe.  Maps clock name to base bit in
 * SCISPICKCR. */
...
__STATIC_INLINE uint32_t R_FSP_SystemClockHzGet(fsp_priv_clock_t clock) {
  ...
  uint32_t sckdivcr =
      FSP_STYPE3_REG32_READ(R_SYSTEM->SCKDIVCR, BSP_CFG_CLOCKS_SECURE);
  ...
  /* Get CPUCLK divisor */
  uint32_t cpuclk_div =
      FSP_STYPE3_REG8_READ(R_SYSTEM->SCKDIVCR2, BSP_CFG_CLOCKS_SECURE) &
...
__STATIC_INLINE uint32_t R_FSP_SciSpiClockHzGet(void) {
  ...
  uint32_t clock_div =
      R_FSP_ClockDividerGet(scispidivcr & FSP_PRV_SCKDIVCR_DIV_MASK);
  fsp_priv_source_clock_t scispicksel =
      (fsp_priv_source_clock_t)R_SYSTEM->SCISPICKCR_b.SCISPICKSEL;
...
__STATIC_INLINE uint32_t R_FSP_SpiClockHzGet(void) {
  uint32_t spidivcr =
      FSP_STYPE3_REG8_READ(R_SYSTEM->SPICKDIVCR, BSP_CFG_CLOCKS_SECURE);
  uint32_t clock_div =
      R_FSP_ClockDividerGet(spidivcr & FSP_PRV_SCKDIVCR_DIV_MASK);
  fsp_priv_source_clock_t spicksel =
      (fsp_priv_source_clock_t)((FSP_STYPE3_REG8_READ(R_SYSTEM->SPICKCR,
                                                      BSP_CFG_CLOCKS_SECURE) &
                                 R_SYSTEM_SPICKCR_CKSEL_Msk) >>
...
__STATIC_INLINE uint32_t R_FSP_SciClockHzGet(void) {
  uint32_t scidivcr =
      FSP_STYPE3_REG8_READ(R_SYSTEM->SCICKDIVCR, BSP_CFG_CLOCKS_SECURE);
  uint32_t clock_div =
      R_FSP_ClockDividerGet(scidivcr & FSP_PRV_SCKDIVCR_DIV_MASK);
...
__STATIC_INLINE void R_BSP_FlashCacheEnable(void) {
  ...
  /* Invalidate the flash cache and wait until it is invalidated. (See
   * section 55.3.2.2 "Operation" of the Flash Cache in the RA6M3 manual
   * R01UH0878EJ0100). */
...
/** Prototype of default function called before errors are returned in FSP code
 * if BSP_CFG_LOG_ERRORS is set to 1. */
void fsp_error_log(fsp_err_t err, const char *file, int32_t line);

#endif

/** In the event of an unrecoverable error the BSP will by default call the
 * __BKPT() intrinsic function which will alert the user of the error. The user
 * can override this default behavior by defining their own
