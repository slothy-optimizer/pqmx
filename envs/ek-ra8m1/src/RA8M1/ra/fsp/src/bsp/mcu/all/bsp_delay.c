...
BSP_SECTION_FLASH_GAP void R_BSP_SoftwareDelay(uint32_t delay,
  ...
  uint32_t total_us =
      (delay * units); /** Convert the requested time to microseconds. */
  uint64_t ns_64bits;

  iclk_hz = SystemCoreClock; /** Get the system clock frequency in Hz. */

  /* Running on the Sub-clock (32768 Hz) there are 30517 ns/cycle. This means
   * one cycle takes 31 us. One execution loop of the delay_loop takes 6 cycles
   * which at 32768 Hz is 180 us. That does not include the overhead below prior
   * to even getting to the delay loop. Given this, at this frequency anything
   * less then a delay request of 122 us will not even generate a single pass
   * through the delay loop.  For this reason small delays (<=~200 us) at this
   * slow clock rate will not be possible and such a request will generate a
   * minimum delay of ~200 us.*/
  ns_per_cycle =
      BSP_DELAY_NS_PER_SECOND / iclk_hz; /** Get the # of nanoseconds/cycle. */

  /* We want to get the time in total nanoseconds but need to be conscious of
   * overflowing 32 bits. We also do not want to do 64 bit */
  /* division as that pulls in a division library. */
  ns_64bits =
      (uint64_t)total_us * (uint64_t)BSP_DELAY_NS_PER_US;  // Convert to ns.
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
BSP_SECTION_FLASH_GAP BSP_ATTRIBUTE_STACKLESS void bsp_prv_software_delay_loop(
    __attribute__((unused)) uint32_t loop_cnt) {
  __asm volatile(
#if defined(RENESAS_CORTEX_M85) && \
    (defined(__ARMCC_VERSION) || defined(__GNUC__))

      /* Align the branch target to a 64-bit boundary, a CM85 specific
         optimization. */
    ...
    (defined(__llvm__) && !defined(__CLANG_TIDY__))
      "   subs r0, #1         \n"  ///< 1 cycle
