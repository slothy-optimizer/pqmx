...
bsp_grp_irq_cb_t
    g_bsp_group_irq_sources[BSP_GRP_IRQ_TOTAL_ITEMS] BSP_SECTION_EARLY_INIT;
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
BSP_SECTION_FLASH_GAP fsp_err_t
R_BSP_GroupIrqWrite(bsp_grp_irq_t irq, void (*p_callback)(bsp_grp_irq_t irq)) {
...
BSP_SECTION_FLASH_GAP void NMI_Handler(void) {
  /* NMISR is masked by NMIER to prevent iterating over NMI status flags that
   * are not enabled. */
  uint16_t nmier = R_ICU->NMIER;
  uint16_t nmisr = R_ICU->NMISR & nmier;

  /* Loop over all NMI status flags */
  for (bsp_grp_irq_t irq = BSP_GRP_IRQ_IWDT_ERROR;
       irq <= (bsp_grp_irq_t)(BSP_GRP_IRQ_TOTAL_ITEMS - 1); irq++) {
  ...
  /* Wait for NMISR to be cleared before exiting the ISR to prevent the IRQ from
   * being regenerated. See section "13.2.12 NMICLR : Non-Maskable Interrupt
   * Status Clear Register" in the RA8M1 manual R01UH0994EJ0100 */
