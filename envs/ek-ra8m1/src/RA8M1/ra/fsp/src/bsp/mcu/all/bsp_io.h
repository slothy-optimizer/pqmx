...
__STATIC_INLINE void R_BSP_PinWrite(bsp_io_port_pin_t pin,
  ...
  uint32_t pfs_bits =
      R_PFS->PORT[pin >> 8].PIN[pin & BSP_IO_PRV_8BIT_MASK].PmnPFS;
  ...
  R_PFS->PORT[pin >> 8].PIN[pin & BSP_IO_PRV_8BIT_MASK].PmnPFS =
      (uint16_t)(BSP_IO_PFS_PDR_OUTPUT | lvl);
#else
  R_PFS->PORT[pin >> 8].PIN[pin & BSP_IO_PRV_8BIT_MASK].PmnPFS =
      (BSP_IO_PFS_PDR_OUTPUT | lvl);
#endif
}
...
__STATIC_INLINE void R_BSP_PinAccessEnable(void) {
  ...
  if (0 == g_protect_pfswe_counter) {
    ...
    (BSP_FEATURE_TZ_VERSION == 2 && FSP_PRIV_TZ_USE_SECURE_REGS)
    R_PMISC->PWPRS = 0;  ///< Clear BOWI bit - writing to PFSWE bit enabled
    R_PMISC->PWPRS =
        1U << BSP_IO_PWPR_PFSWE_OFFSET;  ///< Set PFSWE bit - writing to PFS
                                         ///< register enabled
#else
...
__STATIC_INLINE void R_BSP_PinAccessDisable(void) {
  ...
  if (0 == g_protect_pfswe_counter) {
    ...
    R_PMISC->PWPRS =
        1U << BSP_IO_PWPR_B0WI_OFFSET;  ///< Set BOWI bit - writing to PFS
                                        ///< register enabled
#else
    R_PMISC->PWPR = 0;  ///< Clear PFSWE bit - writing to PFS register disabled
    R_PMISC->PWPR = 1U << BSP_IO_PWPR_B0WI_OFFSET;  ///< Set BOWI bit - writing
                                                    ///< to PFSWE bit disabled
#endif
  }
