 ...
 * Macro definitions
 **********************************************************************************************************************/
#define BSP_ICU_VECTOR_MAX_ENTRIES \
  (BSP_VECTOR_TABLE_MAX_ENTRIES - BSP_CORTEX_VECTOR_TABLE_ENTRIES)
...
__STATIC_INLINE void R_FSP_IsrContextSet(IRQn_Type const irq, void *p_context) {
  /* This provides access to the ISR context array defined in bsp_irq.c. This is
   * an inline function instead of being part of bsp_irq.c for performance
   * considerations because it is used in interrupt service routines. */
...
__STATIC_INLINE void R_BSP_IrqStatusClear(IRQn_Type irq) {
   ...
   * See section "13.5.1 Operations During an Interrupt" in the RA8M1 manual
   * R01UH0994EJ0100. */
...
__STATIC_INLINE void R_BSP_IrqClearPending(IRQn_Type irq) {
  ...
  /* Flush memory transactions to ensure that the IR bit is cleared before
   * clearing the pending bit in the NVIC. */
  __DMB();
#endif

  /* The following statement is used in place of NVIC_ClearPendingIRQ to avoid
   * including a branch for system exceptions every time an interrupt is cleared
   * in the NVIC. */
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       * @warning Do not call this function for system exceptions where the IRQn_Type value is < 0.
                                                                                                                       **********************************************************************************************************************/
__STATIC_INLINE void R_BSP_IrqCfg(IRQn_Type const irq, uint32_t priority,
                                  void *p_context) {
  /* The following statement is used in place of NVIC_SetPriority to avoid
   * including a branch for system exceptions every time a priority is
   * configured in the NVIC. */
#if (4U == __CORTEX_M)
  NVIC->IP[((uint32_t)irq)] =
      (uint8_t)((priority << (8U - __NVIC_PRIO_BITS)) & (uint32_t)UINT8_MAX);
#elif (33 == __CORTEX_M)
  NVIC->IPR[((uint32_t)irq)] =
      (uint8_t)((priority << (8U - __NVIC_PRIO_BITS)) & (uint32_t)UINT8_MAX);
#elif (23 == __CORTEX_M)
  NVIC->IPR[_IP_IDX(irq)] =
      ((uint32_t)(NVIC->IPR[_IP_IDX(irq)] &
                  ~((uint32_t)UINT8_MAX << _BIT_SHIFT(irq))) |
       (((priority << (8U - __NVIC_PRIO_BITS)) & (uint32_t)UINT8_MAX)
        << _BIT_SHIFT(irq)));
#else
...
__STATIC_INLINE void R_BSP_IrqEnableNoClear(IRQn_Type const irq) {
  /* The following statement is used in place of NVIC_EnableIRQ to avoid
   * including a branch for system exceptions every time an interrupt is enabled
   * in the NVIC. */
  uint32_t _irq = (uint32_t)irq;
...
__STATIC_INLINE void R_BSP_IrqDisable(IRQn_Type const irq) {
  /* The following statements is used in place of NVIC_DisableIRQ to avoid
   * including a branch for system exceptions every time an interrupt is cleared
   * in the NVIC. */
  uint32_t _irq = (uint32_t)irq;
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       * @warning Do not call this function for system exceptions where the IRQn_Type value is < 0.
                                                                                                                       **********************************************************************************************************************/
__STATIC_INLINE void R_BSP_IrqCfgEnable(IRQn_Type const irq, uint32_t priority,
                                        void *p_context) {
...
void R_BSP_IrqCfgEnable(IRQn_Type const irq, uint32_t priority,
                        void *p_context);
