...
static void r_macl_mul_q31(const q31_t *p_src_a, const q31_t *p_src_b,
                           q31_t *p_dst, uint32_t block_size);

static void r_macl_scale_q31(const q31_t *p_src, q31_t scale_fract,
                             int8_t shift, q31_t *p_dst, uint32_t block_size);
...
static void r_macl_mat_mul_acc_q31(const q31_t *p_in_a, const q31_t *p_in_b,
                                   q31_t *p_out, uint16_t num_cols_a,
                                   uint16_t num_cols_b);

static void r_macl_mat_scale_q31(const arm_matrix_instance_q31 *p_src,
                                 q31_t scale_fract, int32_t shift,
                                 arm_matrix_instance_q31 *p_dst);
static void r_macl_conv_q31(const q31_t *p_src_a, const q31_t *p_src_b,
                            q31_t *p_dst, uint8_t block_size);

static uint32_t r_macl_recip_q31(q31_t in, q31_t *dst,
                                 const q31_t *p_recip_table);
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
void R_BSP_MaclMulQ31(const q31_t *p_src_a, const q31_t *p_src_b, q31_t *p_dst,
                      uint32_t block_size) {
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       * @param[in]   block_size   Numbers of elements to be calculated.
                                                                                                                       **********************************************************************************************************************/
void R_BSP_MaclScaleQ31(const q31_t *p_src, q31_t scale_fract, int8_t shift,
                        q31_t *p_dst, uint32_t block_size) {
  /* Disable Fixed point mode. */
  R_MACL->MULC = BSP_MACL_FIXED_POINT_MODE_DISABLE;
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
void R_BSP_MaclMatVecMulQ31(const arm_matrix_instance_q31 *p_src_mat,
                            const q31_t *p_vec, q31_t *p_dst) {
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
void R_BSP_MaclBiquadCsdDf1Q31(
    const arm_biquad_casd_df1_inst_q31 *p_biquad_csd_df1_inst,
    const q31_t *p_src, q31_t *p_dst, uint32_t block_size) {
  const q31_t *p_coeffs =
      p_biquad_csd_df1_inst->pCoeffs;                 // Coefficient pointer
  ...
  uint32_t shift = ((uint32_t)p_biquad_csd_df1_inst->postShift +
                    1U);  // Shift to be applied to the output
  uint32_t r_shift =
      BSP_MACL_32_BIT - shift;  // Shift to be applied to the output
  volatile uint64_t *p_result_in_q62 =
      (uint64_t *)&(R_MACL->MULR0.MULRL);  // Assign to address of MULR0
  ...
  while (stage > 0U) {
    ...
    while (sample > 0U) {
      ...
      /* Update state buffer */
      p_state[state_update_element + 1U] =
          p_state[state_update_element];  // x[n - 2] = x[n - 1]
      ...
      *p_dst_local = (q31_t)(*p_result_in_q62 >> r_shift);
      ...
      /* Update state buffer */
      p_state[state_update_element + 3U] =
          p_state[state_update_element + 2U];  // y[n - 2] = y[n - 1]
      p_state[state_update_element + 2U] =
          *p_dst_local;  // Update value of y[n-1]
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
void R_BSP_MaclConvQ31(const q31_t *p_src_a, uint32_t src_a_len,
                       const q31_t *p_src_b, uint32_t src_b_len, q31_t *p_dst) {
  ...
  /* The algorithm implementation is based on the lengths of the inputs. src B
   * is always made to slide across src A. Therefore, length of B is always
   * considered as shorter or equal to length of A */
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
arm_status R_BSP_MaclConvPartialQ31(const q31_t *p_src_a, uint32_t src_a_len,
                                    const q31_t *p_src_b, uint32_t src_b_len,
                                    q31_t *p_dst, uint32_t first_idx,
                                    uint32_t num_points) {
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
void R_BSP_MaclFirDecimateQ31(
    const arm_fir_decimate_instance_q31 *p_fir_decimate_ins_q31,
    const q31_t *p_src, q31_t *p_dst, uint32_t block_size) {
  q31_t *p_state = p_fir_decimate_ins_q31->pState;  // State pointer
  const q31_t *p_coeffs =
      p_fir_decimate_ins_q31->pCoeffs;  // Coefficient pointer
  ...
  /* p_fir_decimate_ins_q31->pState buffer contains previous frame (num_taps -
   * 1) samples */
  /* p_state_cur points to the location where the new input data should be
   * written */
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       * @param[in]   block_size                    Numbers of samples to be calculated.
                                                                                                                       **********************************************************************************************************************/
void R_BSP_MaclFirInterpolateQ31(
    const arm_fir_interpolate_instance_q31 *p_fir_interpolate_ins_q31,
    const q31_t *p_src, q31_t *p_dst, uint32_t block_size) {
  /* Disable Fixed point mode. */
  R_MACL->MULC = BSP_MACL_FIXED_POINT_MODE_DISABLE;
  ...
  const q31_t *p_coeffs =
      p_fir_interpolate_ins_q31->pCoeffs;  // Coefficient pointer
  q31_t *p_state_cur;       // Points to the current sample of the state
  ...
  while (blk_cnt > 0U) {
    ...
    *p_state_cur++ = *p_src++;
    ...
    /* Advance the state pointer by 1 to process the next group of interpolation
     * factor number samples */
    p_state = p_state + 1;
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
void R_BSP_MaclCorrelateQ31(const q31_t *p_src_a, uint32_t src_a_len,
                            const q31_t *p_src_b, uint32_t src_b_len,
                            q31_t *p_dst) {
  ...
  /* If src_a_len > src_b_len,
   * (src_a_len - src_b_len) zeroes has to included in the starting of the
   * output buffer */

  /* If src_a_len < src_b_len,
   * (src_b_len - src_a_len) zeroes has to included in the ending of the output
   * buffer */
  if (src_a_len >= src_b_len) {
   ...
   * iteration. In the second stage of the algorithm, src_b_len number of
   * multiplications are done. In the third stage of the algorithm, the
   * multiplications decrease by one for every iteration. The algorithm is
   * implemented in three stages. The loop counters of each stage is initiated
   * here. */
   ...
   * ....
   * sum = x[src_a_len-src_b_len-2] * y[0] + x[src_a_len-src_b_len-1] * y[1]
   * +...+ x[src_a_len-1] * y[src_b_len-1]
   *//* Working pointer of inputA */
  p_in_a_buf = p_in1;
  ...
  /* Stage2 depends on src_b_len as in this stage src_b_len number of MACS are
   * performed. */
  ...
  /* sum += x[src_a_len-src_b_len+1] * y[0] + x[src_a_len-src_b_len+2] * y[1]
   * +...+ x[src_a_len-1] * y[src_b_len-1] sum += x[src_a_len-src_b_len+2] *
   * y[0] + x[src_a_len-src_b_len+3] * y[1] +...+ x[src_a_len-1] *
   * y[src_b_len-1]
   * ....
...
void R_BSP_MaclFirSparseQ31(arm_fir_sparse_instance_q31 *p_fir_sparse_ins_q31,
                            const q31_t *p_src, q31_t *p_dst,
                            q31_t *p_scratch_in, uint32_t block_size) {
  ...
  arm_circularWrite_f32((int32_t *)p_state_tmp, (int32_t)delay_size,
                        &p_fir_sparse_ins_q31->stateIndex, 1, (int32_t *)p_src,
                        1, block_size);

  /* Read Index, from where the state buffer should be read, is calculated. */
  read_index =
      (int32_t)(p_fir_sparse_ins_q31->stateIndex - block_size) - *p_tap_delay++;
  ...
  arm_circularRead_f32((int32_t *)p_state_tmp, (int32_t)delay_size, &read_index,
                       1, (int32_t *)p_scratch_in_tmp,
                       (int32_t *)p_scratch_in_tmp, (int32_t)block_size, 1,
                       block_size);
  ...
  while (tap_cnt > 0U) {
    ...
    /* block_size samples are read from the state buffer */
    arm_circularRead_f32((int32_t *)p_state_tmp, (int32_t)delay_size,
                         &read_index, 1, (int32_t *)p_scratch_in_tmp,
                         (int32_t *)p_scratch_in_tmp, (int32_t)block_size, 1,
                         block_size);
    ...
    /* Read Index, from where the state buffer should be read, is calculated. */
    read_index = (int32_t)(p_fir_sparse_ins_q31->stateIndex - block_size) -
                 *p_tap_delay++;
...
void R_BSP_MaclLmsNormQ31(arm_lms_norm_instance_q31 *p_lms_norm_ins_q31,
                          const q31_t *p_src, q31_t *p_ref, q31_t *p_out,
                          q31_t *p_err, uint32_t block_size) {
  ...
  q31_t one_by_energy;  // Temporary variables to store mu product and
                        // reciprocal of energy
  q31_t post_shift;     // Post shift to be applied to weight after reciprocal
                        // calculation
  q31_t acc_l;          // Low accumulator
  q31_t acc_h;          // High accumulator
  uint32_t u_shift = ((uint32_t)p_lms_norm_ins_q31->postShift + 1U);
  uint32_t l_shift =
      BSP_MACL_32_BIT - u_shift;  // Shift to be applied to the output
  q63_t result_temp;              // Temporary variable to read result register
  volatile uint64_t *p_result_r4 =
      (uint64_t *)&(R_MACL->MULR4.MULRL);  // Assign to address of MULR4
  ...
  /* p_lms_norm_ins_q31->pState points to buffer which contains previous frame
   * (num_taps - 1) samples */
  /* p_state_curnt points to the location where the new input data should be
   * written */
  ...
  while (blk_cnt > 0U) {
    ...
    *p_err++ = err_data;
    ...
    post_shift =
        (q31_t)r_macl_recip_q31((q31_t)energy + DELTA_Q31, &one_by_energy,
...
void R_BSP_MaclLmsQ31(const arm_lms_instance_q31 *p_lms_ins_q31,
  ...
  uint32_t num_taps =
      p_lms_ins_q31->numTaps;  // Number of filter coefficients in the filter
  uint32_t tap_cnt;            // Loop counters
  uint32_t blk_cnt;            // Loop counters
  ...
  /* p_lms_ins_q31->pState points to buffer which contains previous frame
   * (numTaps - 1) samples */
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       * @param[in]     block_size    Number of samples to process
                                                                                                                       **********************************************************************************************************************/
void R_BSP_MaclFirQ31(const arm_fir_instance_q31 *p_fir_inst,
                      const q31_t *p_src, q31_t *p_dst, uint32_t block_size) {
  q31_t *p_state;        // Pointer to state buffer which will be used to hold
                         // calculated sample
  q31_t *p_state_curnt;  // Intermediate pointer used to write sample into state
                         // buffer
  q31_t *p_state_tmp;    // Intermediate pointer to write sample value into
                         // register MAC32S
  const q31_t *p_coeff_tmp;  // Intermediate pointer to write coefficient into
                             // register MULB0
  ...
  /* p_fir_inst->pState points to state array which contains previous frame
   * (num_taps - 1) samples */
  /* p_state_curnt points to the location where the new input data should be
   * written */
  ...
  while (blk_cnt > 0U) {
    ...
    *p_state_curnt++ = *p_src++;
    ...
    do {
      /* y[n] =  b[numTaps-1] * x[n-numTaps-1] + b[numTaps-2] * x[n-numTaps-2] +
  ...
  /* Processing is complete. Now copy the last num_taps - 1 samples to the start
   * of the state buffer. This prepares the state buffer for the next function
   * call. */
...
static void r_macl_mul_q31(const q31_t *p_src_a, const q31_t *p_src_b,
  ...
  while (block_size_cnt > 0U) {
    if ((*p_src_a_local == (q31_t)BSP_MACL_Q31_MIN_VALUE) &&
        (*p_src_b_local == (q31_t)BSP_MACL_Q31_MIN_VALUE)) {
      /* Overflow case */
...
static void r_macl_mat_mul_q31(const arm_matrix_instance_q31 *p_src_a,
  ...
  uint16_t num_cols_b =
      p_src_b->numCols;  // Number of columns of input matrix B
  uint16_t num_cols_a =
      p_src_a->numCols;       // Number of columns of input matrix A
  ...
  do {
    ...
    /* For every row wise process, p_in_b pointer is set to starting address of
     * p_src_b data */
    p_in_b = p_src_b->pData;

    /* Column loop */
    do {
      /* Perform the multiply-accumulates a row in p_src_a with a column in
       * p_src_b */
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
void r_macl_conv_q31(const q31_t *p_src_a, const q31_t *p_src_b, q31_t *p_dst,
                     uint8_t block_size) {
  uint8_t cnt;
  const q31_t *p_src_a_local;
  const q31_t *p_src_b_local;
