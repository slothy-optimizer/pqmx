...
/** Masks for setting or clearing the PRCR register. Use -1 for size because
 * PWPR in MPC is used differently. */
...
BSP_SECTION_FLASH_GAP void R_BSP_RegisterProtectEnable(
    bsp_reg_protect_t regs_to_protect) {
  ...
  if (0U == g_protect_counters[regs_to_protect]) {
     ...
     *
     * When writing to the PRCR register the upper 8-bits must be the correct
     * key. Set lower bits to 0 to disable writes. */
#if BSP_TZ_NONSECURE_BUILD && BSP_FEATURE_TZ_VERSION == 2
    R_SYSTEM->PRCR_NS = ((R_SYSTEM->PRCR_NS | BSP_PRV_PRCR_KEY) &
                         (uint16_t)(~g_prcr_masks[regs_to_protect]));
#else
    R_SYSTEM->PRCR = ((R_SYSTEM->PRCR | BSP_PRV_PRCR_KEY) &
                      (uint16_t)(~g_prcr_masks[regs_to_protect]));
#endif
  }
...
BSP_SECTION_FLASH_GAP void R_BSP_RegisterProtectDisable(
    bsp_reg_protect_t regs_to_unprotect) {
  /** Get/save the current state of interrupts */
  FSP_CRITICAL_SECTION_DEFINE;
  FSP_CRITICAL_SECTION_ENTER;
...
#else
    R_SYSTEM->PRCR =
        ((R_SYSTEM->PRCR | BSP_PRV_PRCR_KEY) | g_prcr_masks[regs_to_unprotect]);
#endif
  }
