...
caddr_t _sbrk(int incr) {
  ...
  /* The returned address must be aligned to a word boundary to prevent hard
   * faults on cores that do not support unaligned access. We assume the heap
   * starts on a word boundary and make sure all allocations are a multiple
   * of 4. */
  bytes = (bytes + 3U) & (~3U);
