...
#define IOPORT_PRV_CLEAR_BITS_MASK \
  (0x1F01FCD5U)  ///< Zero bits in mask must be written as zero to PFS register
...
#define IOPORT_PRV_PORT_ADDRESS(port_number) \
  ((uint32_t)(R_PORT1 - R_PORT0) * (port_number) + R_PORT0)
...
static void bsp_vbatt_init(
    ioport_cfg_t const *const p_pin_cfg);  // Used internally by BSP
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
fsp_err_t R_IOPORT_PinCfg(ioport_ctrl_t *const p_ctrl, bsp_io_port_pin_t pin,
                          uint32_t cfg) {
#if (1 == IOPORT_CFG_PARAM_CHECKING_ENABLE)
  ioport_instance_ctrl_t *p_instance_ctrl = (ioport_instance_ctrl_t *)p_ctrl;
  FSP_ASSERT(NULL != p_instance_ctrl);
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       * @note This function is re-entrant for different pins.
                                                                                                                       **********************************************************************************************************************/
fsp_err_t R_IOPORT_PinRead(ioport_ctrl_t *const p_ctrl, bsp_io_port_pin_t pin,
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
fsp_err_t R_IOPORT_PortRead(ioport_ctrl_t *const p_ctrl, bsp_io_port_t port,
  ...
  R_PORT0_Type *p_ioport_regs = IOPORT_PRV_PORT_ADDRESS(
      (port >> IOPORT_PRV_PORT_OFFSET) & IOPORT_PRV_8BIT_MASK);
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
fsp_err_t R_IOPORT_PortWrite(ioport_ctrl_t *const p_ctrl, bsp_io_port_t port,
...
/*******************************************************************************************************************/ /**
                                                                                                                       ...
                                                                                                                       **********************************************************************************************************************/
fsp_err_t R_IOPORT_PinWrite(ioport_ctrl_t *const p_ctrl, bsp_io_port_pin_t pin,
  ...
  bsp_io_port_t port =
      (bsp_io_port_t)(IOPORT_PRV_PORT_BITS & (ioport_size_t)pin);
  ...
  /* PCNTR register is updated instead of using PFS as access is atomic and PFS
   * requires separate enable/disable using PWPR register */
...
fsp_err_t R_IOPORT_PortEventInputRead(ioport_ctrl_t *const p_ctrl,
  ...
  FSP_ERROR_RETURN((BSP_FEATURE_IOPORT_ELC_PORTS & (1 << port_number)),
  ...
  /* Read current value of EIDR value from PCNTR2 register for the specified
   * port */
...
fsp_err_t R_IOPORT_PinEventInputRead(ioport_ctrl_t *const p_ctrl,
  ...
  /* Get the port address */
  R_PORT0_Type *p_ioport_regs = IOPORT_PRV_PORT_ADDRESS(
      (pin >> IOPORT_PRV_PORT_OFFSET) & IOPORT_PRV_8BIT_MASK);
...
fsp_err_t R_IOPORT_PinEventOutputWrite(ioport_ctrl_t *const p_ctrl,
  ...
  FSP_ERROR_RETURN(IOPORT_OPEN == p_instance_ctrl->open, FSP_ERR_NOT_OPEN);
  FSP_ERROR_RETURN(
      (pin_value == BSP_IO_LEVEL_HIGH) || (pin_value == BSP_IO_LEVEL_LOW),
      FSP_ERR_INVALID_ARGUMENT);
  uint32_t port_number = pin >> IOPORT_PRV_PORT_OFFSET;
  FSP_ERROR_RETURN((BSP_FEATURE_IOPORT_ELC_PORTS & (1 << port_number)),
                   FSP_ERR_INVALID_ARGUMENT);
  ...
  r_ioport_hw_pin_event_output_data_write(
      (bsp_io_port_t)(pin & IOPORT_PRV_PORT_BITS),
...
void r_ioport_pins_config(const ioport_cfg_t *p_cfg) {
  ...
  for (pin_count = 0U; pin_count < p_pin_data->number_of_pins; pin_count++) {
    r_ioport_pfs_write(p_pin_data->p_pin_cfg_data[pin_count].pin,
...
static void r_ioport_pfs_write(bsp_io_port_pin_t pin, uint32_t value) {
  ...
  /* PMR bits should be cleared before specifying PSEL. Reference section "20.7
   * Notes on the PmnPFS Register Setting" in the RA6M3 manual R01UH0886EJ0100.
   */
  if ((value & IOPORT_PRV_PERIPHERAL_FUNCTION) > 0) {
    /* Clear PMR */
    R_PFS->PORT[pin >> IOPORT_PRV_PORT_OFFSET]
        .PIN[pin & BSP_IO_PRV_8BIT_MASK]
        .PmnPFS_b.PMR = 0;

    /* New config with PMR = 0 */
    R_PFS->PORT[pin >> IOPORT_PRV_PORT_OFFSET]
        .PIN[pin & BSP_IO_PRV_8BIT_MASK]
        .PmnPFS = (value & ~((uint32_t)IOPORT_PRV_PERIPHERAL_FUNCTION));
  }
...
static void bsp_vbatt_init(ioport_cfg_t const *const p_pin_cfg) {
  ...
  if (1 == R_SYSTEM->BBFSAR_b.NONSEC2) {
    /* If security attribution of VBTICTLR is set to non-secure, then use the
     * non-secure alias. */
...
#endif
  {
    /* If security attribution of RTC is set to non-secure, then use the
     * non-secure alias. */
  ...
  for (pin_index = 0U; pin_index < p_pin_cfg->number_of_pins; pin_index++) {
    ...
    for (vbatt_index = 0U; vbatt_index < (sizeof(g_vbatt_pins_input) /
                                          sizeof(g_vbatt_pins_input[0]));
         vbatt_index++) {
      if (p_pin_cfg->p_pin_cfg_data[pin_index].pin ==
          g_vbatt_pins_input[vbatt_index]) {
        /* Get PSEL value for pin. */
        uint32_t pfs_psel_value = p_pin_cfg->p_pin_cfg_data[pin_index].pin_cfg &
                                  BSP_PRV_PFS_PSEL_MASK;

        /* Check if pin is being used for RTC or AGT use. */
        if ((IOPORT_PERIPHERAL_AGT == pfs_psel_value) ||
            (IOPORT_PERIPHERAL_CLKOUT_COMP_RTC == pfs_psel_value)) {
          /* Bit should be set to 1. */
#if BSP_FEATURE_SYSC_HAS_VBTICTLR
#if BSP_TZ_NONSECURE_BUILD
          if (0 == R_SYSTEM->BBFSAR_b.NONSEC2) {
            /* Do nothing: non secure build can't configure secure VBTICTLR
             * register. */
          } else
#endif
...
#endif
          {
            /* Do nothing: non secure build can't configure secure RTC
             * registers. */
          } else
#endif
          {
            if (0 == p_rtc->RTCCR[vbatt_index].RTCCR_b.TCEN) {
              ...
              R_BSP_SoftwareDelay(BSP_PRV_RTC_RESET_DELAY_US,
