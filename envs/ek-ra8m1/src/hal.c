...
static FILE __stdio =
    FDEV_SETUP_STREAM(trace_putc, null_getc, NULL, _FDEV_SETUP_RW);
...
void hal_pmu_enable() {
  ...
  // The Cortex-M85 TRM claims it shouldn't be enabled at boot, but apparently
  // it is
...
void hal_pmu_send_stats(char *s, pmu_stats const *stats) {
  ...
  printf("- instructions:         %lu (IPC=0.%02lu)\n",
  ...
  printf(
      "- MVE LSU instructions: %lu (%lu%% of MVE instructions, busy %lu%% of "
      "cycles)\n",
  ...
  printf(
      "- MVE INT instructions: %lu (%lu%% of MVE instructions, busy %lu%% of "
      "cycles)\n",
  ...
  printf(
      "- MVE MUL instructions: %lu (%lu%% of MVE instructions, busy %lu%% of "
      "cycles)\n",
  ...
  printf(
      "- MVE resource stalls:  %lu (%lu%% of MVE stalls)\n",
      (unsigned long)stats->stall_mve_resource,
      (unsigned long)(stats->stall_mve_resource * 100) / stats->stall_mve_all);

  printf(
      "s %s  %lu %lu  %lu %lu  %lu %lu  %lu %lu %lu  %lu %lu %lu  %lu %lu %lu  "
      "%lu %lu  %lu %lu %lu\n",
      s, (unsigned long)stats->pmu_cycles, (unsigned long)stats->inst_all,
      (unsigned long)(stats->inst_all * 100) / stats->pmu_cycles,
      (unsigned long)stats->stall_all,
      (unsigned long)(stats->stall_all * 100) / stats->inst_all,
      (unsigned long)stats->inst_mve_all,
