...
int crypto_kem_keypair(unsigned char *pk, unsigned char *sk)
{
  ...
  sha3_256_wrap( sk + SABER_SECRETKEYBYTES - 64,
            pk, SABER_INDCPA_PUBLICKEYBYTES ); // Then hash(pk) is appended.

  randombytes_saber( sk + SABER_SECRETKEYBYTES - SABER_KEYBYTES, SABER_KEYBYTES ); // Remaining part of sk contains a pseudo-random number.
...
int crypto_kem_enc(unsigned char *c, unsigned char *k, const unsigned char *pk)
{
  ...
  randombytes_saber(buf, 32);

  sha3_256_wrap(buf, buf, 32); // BUF[0:31] <-- random message (will be used as the key for client) Note: hash doesnot release system RNG output

  sha3_256_wrap(buf + 32, pk, SABER_INDCPA_PUBLICKEYBYTES); // BUF[32:63] <-- Hash(public key);  Multitarget countermeasure for coins + contributory KEM

  sha3_512_wrap(kr, buf, 64);               // kr[0:63] <-- Hash(buf[0:63]);
                                       // K^ <-- kr[0:31]
                                       // noiseseed (r) <-- kr[32:63];
  indcpa_kem_enc(buf, kr + 32, pk, c); // buf[0:31] contains message; kr[32:63] contains randomness r;

  sha3_256_wrap(kr + 32, c, SABER_BYTES_CCA_DEC);

  sha3_256_wrap(k, kr, 64); // hash concatenation of pre-k and h(c) to k
...
int crypto_kem_dec(unsigned char *k, const unsigned char *c, const unsigned char *sk)
{
  ...
  sha3_512_wrap(kr, buf, 64);

  indcpa_kem_enc(buf, kr + 32, pk, cmp);

  fail = verify(c, cmp, SABER_BYTES_CCA_DEC);

  sha3_256_wrap(kr + 32, c, SABER_BYTES_CCA_DEC); // overwrite coins in kr with h(c)
